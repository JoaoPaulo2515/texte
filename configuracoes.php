...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['acao']) && $_POST['acao'] == 'salvar_inss') {
    ...
    for ($i = 0; $i < count($faixas); $i++) {
        $tabela_json[] = [
            'faixa' => $faixas[$i],
            'limite' => floatval(str_replace('.', '', $limites[$i])),
            'aliquota' => floatval($aliquotas[$i]),
            'deducao' => floatval(str_replace('.', '', $deducoes[$i]))
        ];
    }
    
    $stmt = $conn->prepare("
        INSERT INTO escola_parametros_sistema (escola_id, parametro, valor)
        VALUES (:escola_id, 'folha_tabela_inss', :valor)
        ON DUPLICATE KEY UPDATE valor = :valor
    ");
    $stmt->execute([
        ':escola_id' => $escola_id,
        ':valor' => json_encode($tabela_json)
    ]);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['acao']) && $_POST['acao'] == 'salvar_irrf') {
    ...
    $stmt = $conn->prepare("
        INSERT INTO escola_parametros_sistema (escola_id, parametro, valor)
        VALUES (:escola_id, 'folha_tabela_irrf', :valor)
        ON DUPLICATE KEY UPDATE valor = :valor
    ");
    $stmt->execute([
        ':escola_id' => $escola_id,
...
if (isset($_GET['restaurar']) && $_GET['restaurar'] == 'inss') {
    ...
    $stmt->execute([
        ':escola_id' => $escola_id,
        ':valor' => json_encode($tabela_inss_padrao)
    ]);
...
if (isset($_GET['restaurar']) && $_GET['restaurar'] == 'irrf') {
    ...
    $stmt->execute([
        ':escola_id' => $escola_id,
        ':valor' => json_encode($tabela_irrf_padrao)
    ]);
...
$stmt = $conn->prepare("
    SELECT valor FROM escola_parametros_sistema 
    WHERE escola_id = :escola_id AND parametro = 'folha_tabela_inss'
");
...
$stmt = $conn->prepare("
    SELECT valor FROM escola_parametros_sistema 
    WHERE escola_id = :escola_id AND parametro = 'folha_tabela_irrf'
");
$stmt->execute([':escola_id' => $escola_id]);
$row = $stmt->fetch(PDO::FETCH_ASSOC);
