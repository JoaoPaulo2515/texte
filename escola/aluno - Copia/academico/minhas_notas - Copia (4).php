...
// ============================================
// VERIFICAÇÃO FINANCEIRA (VERSÃO SIMPLIFICADA)
// ============================================

// 1. Verificar se o boletim foi pago
$boletim_pago = false;
try {
    // Buscar ID do tipo "Boletim"
...
try {
    ...
    $dividas_outros = $stmt_outros->fetch(PDO::FETCH_ASSOC);
    $dividas_outros = $dividas_outros['valor'] ?? 0;
    $valor_divida_outros = $dividas_outros['valor'] ?? 0;
    
    // Dívidas em mensalidades
    ...
    $tota_divida=($dividas_outros+$valor_divida);
...
foreach ($notas_raw as $nota) {
    ...
    $media_bimestre = $nota['media_parcial'] ?? null;
    ...
    // Armazenar média por bimestre
    if ($media_bimestre !== null) {
        $disciplinas[$disciplina]['media_parcial_bimestres'][$bimestre] = $media_bimestre;
    }
...
// Calcular médias parciais por disciplina (média dos bimestres)
foreach ($disciplinas as $disciplina => $dados) {
    $qtd_bimestres = count($dados['bimestres']);
    if ($qtd_bimestres > 0 && $dados['mac_total'] > 0) {
...
foreach ($disciplinas as $disciplina => $dados) {
    ...
    // Só conta como aprovado/reprovado se o boletim estiver liberado
    if ($boletim_liberado) {
        if ($ciclo == 1) {
...
<div class="main-content-aluno">
    ...
    <!-- ALERTA FINANCEIRO - Condição de visualização -->
    ...
    <div class="alert alert-danger alert-financeiro mb-4 fade-in" style="border-left-color: #dc3545;">
        <div class="d-flex align-items-center">
            ...
            <div>
                ...
                Você possui dívidas pendentes no valor de <strong><?php echo $boletim_pago; ?> Kz</strong>.
    ...
    <!-- Cards de Estatísticas (ocultas se bloqueado) -->
    <div class="row g-3 mb-4 fade-in">
        <div class="col-md-3">
                    ...
                    <table class="table table-bordered table-notas align-middle">
                        ...
                        <tbody>
                            ...
                            <tr <?php echo $bloqueado ? 'class="bloqueado"' : ''; ?>>
                                ...
                                <!-- Bimestre 1 -->
                                <td class="text-center"><?php echo formatarNota($dados['bimestres'][1]['mac'] ?? null, $bloqueado); ?></td>
                                <td class="text-center"><?php echo formatarNota($dados['bimestres'][1]['npt'] ?? null, $bloqueado); ?></td>
                                <!-- Bimestre 2 -->
                                <td class="text-center"><?php echo formatarNota($dados['bimestres'][2]['mac'] ?? null, $bloqueado); ?></td>
                                <td class="text-center"><?php echo formatarNota($dados['bimestres'][2]['npt'] ?? null, $bloqueado); ?></td>
                                <!-- Bimestre 3 (bloqueado se parcial ou total) -->
                                <td class="text-center"><?php echo formatarNota($dados['bimestres'][3]['mac'] ?? null, ($bloqueado || $parcial)); ?></td>
                                <td class="text-center"><?php echo formatarNota($dados['bimestres'][3]['npt'] ?? null, ($bloqueado || $parcial)); ?></td>
                                <!-- Bimestre 4 -->
                                <td class="text-center"><?php echo formatarNota($dados['bimestres'][4]['mac'] ?? null, ($bloqueado || $parcial)); ?></td>
                                <td class="text-center"><?php echo formatarNota($dados['bimestres'][4]['npt'] ?? null, ($bloqueado || $parcial)); ?></td>
                                <!-- Bimestre 5 -->
                                <td class="text-center"><?php echo formatarNota($dados['bimestres'][5]['mac'] ?? null, ($bloqueado || $parcial)); ?></td>
                                <td class="text-center"><?php echo formatarNota($dados['bimestres'][5]['npt'] ?? null, ($bloqueado || $parcial)); ?></td>
                                <!-- Bimestre 6 -->
                                <td class="text-center"><?php echo formatarNota($dados['bimestres'][6]['mac'] ?? null, ($bloqueado || $parcial)); ?></td>
                                <td class="text-center"><?php echo formatarNota($dados['bimestres'][6]['npt'] ?? null, ($bloqueado || $parcial)); ?></td>
                                <!-- Média Parcial (bloqueada se bloqueado) -->
                                ...
                                <td class="text-center">
                                    <?php echo getStatusBadge($dados['media_final'] ?: $dados['media_parcial'], $ciclo, $bloqueado); ?>
