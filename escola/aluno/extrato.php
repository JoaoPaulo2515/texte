...
// 1. DÉBITOS - Mensalidades, taxas, etc. (da tabela pagamentos)
$sql_debitos = "SELECT 
                    'debito' as tipo,
                    p.id,
                    p.tipo_pagamento,
...
// 2. DÉBITOS DE outros_pagamentos (pendentes)
$sql_debitos_outros = "SELECT 
                    'debito' as tipo,
                    op.id,
...
// 3. CRÉDITOS - Pagamentos realizados (tabela pagamentos)
$sql_creditos = "SELECT 
                    'credito' as tipo,
                    p.id,
                    p.tipo_pagamento,
                    ...
                    p.quem_pagou
                FROM pagamentos p
                WHERE p.assinatura_id = :aluno_id3 
                AND p.escola_id = :escola_id3
                AND p.status IN ('pago', 'confirmado')";

// 4. CRÉDITOS DE outros_pagamentos (pagos)
$sql_creditos_outros = "SELECT 
                    'credito' as tipo,
                    op.id,
                    op.tipo_pagamento_id,
                    NULL as tipo_pagamento,
                    ...
                    NULL as quem_pagou
                FROM outros_pagamentos op
                WHERE op.aluno_id = :aluno_id4 
                AND op.escola_id = :escola_id4
                AND op.status IN ('pago', 'confirmado')";

// Aplicar filtros
if ($ano_filtro > 0) {
    $sql_debitos .= " AND YEAR(p.data_vencimento) = :ano";
    $sql_debitos_outros .= " AND YEAR(op.data_vencimento) = :ano2";
    $sql_creditos .= " AND YEAR(p.data_pagamento) = :ano3";
    $sql_creditos_outros .= " AND YEAR(op.data_pagamento) = :ano4";
}
if ($mes_filtro > 0) {
    $sql_debitos .= " AND MONTH(p.data_vencimento) = :mes";
    $sql_debitos_outros .= " AND MONTH(op.data_vencimento) = :mes2";
    $sql_creditos .= " AND MONTH(p.data_pagamento) = :mes3";
    $sql_creditos_outros .= " AND MONTH(op.data_pagamento) = :mes4";
}
if (!empty($data_inicio) && !empty($data_fim)) {
    $sql_debitos .= " AND p.data_vencimento BETWEEN :data_inicio AND :data_fim";
    $sql_debitos_outros .= " AND op.data_vencimento BETWEEN :data_inicio2 AND :data_fim2";
    $sql_creditos .= " AND p.data_pagamento BETWEEN :data_inicio3 AND :data_fim3";
    $sql_creditos_outros .= " AND op.data_pagamento BETWEEN :data_inicio4 AND :data_fim4";
}

$sql_debitos .= " ORDER BY data ASC";
$sql_debitos_outros .= " ORDER BY data ASC";
$sql_creditos .= " ORDER BY data ASC";
$sql_creditos_outros .= " ORDER BY data ASC";

$stmt_debitos = $conn->prepare($sql_debitos);
$stmt_debitos_outros = $conn->prepare($sql_debitos_outros);
$stmt_creditos = $conn->prepare($sql_creditos);
$stmt_creditos_outros = $conn->prepare($sql_creditos_outros);

// ==============================================
// EXECUTAR QUERIES COM PARÂMETROS CORRETOS
// ==============================================

// Parâmetros para débitos (pagamentos)
$params_debitos = [
    ':aluno_id' => $aluno_id,
    ':escola_id' => $escola_id
];
if ($ano_filtro > 0) $params_debitos[':ano'] = $ano_filtro;
if ($mes_filtro > 0) $params_debitos[':mes'] = $mes_filtro;
if (!empty($data_inicio) && !empty($data_fim)) {
    $params_debitos[':data_inicio'] = $data_inicio;
    $params_debitos[':data_fim'] = $data_fim;
}

// Parâmetros para débitos (outros_pagamentos)
$params_debitos_outros = [
    ':aluno_id2' => $aluno_id,
    ':escola_id2' => $escola_id
];
if ($ano_filtro > 0) $params_debitos_outros[':ano2'] = $ano_filtro;
if ($mes_filtro > 0) $params_debitos_outros[':mes2'] = $mes_filtro;
if (!empty($data_inicio) && !empty($data_fim)) {
    $params_debitos_outros[':data_inicio2'] = $data_inicio;
    $params_debitos_outros[':data_fim2'] = $data_fim;
}

// Parâmetros para créditos (pagamentos)
$params_creditos = [
    ':aluno_id3' => $aluno_id,
    ':escola_id3' => $escola_id
];
if ($ano_filtro > 0) $params_creditos[':ano3'] = $ano_filtro;
if ($mes_filtro > 0) $params_creditos[':mes3'] = $mes_filtro;
if (!empty($data_inicio) && !empty($data_fim)) {
    $params_creditos[':data_inicio3'] = $data_inicio;
    $params_creditos[':data_fim3'] = $data_fim;
}

// Parâmetros para créditos (outros_pagamentos)
$params_creditos_outros = [
    ':aluno_id4' => $aluno_id,
    ':escola_id4' => $escola_id
];
if ($ano_filtro > 0) $params_creditos_outros[':ano4'] = $ano_filtro;
if ($mes_filtro > 0) $params_creditos_outros[':mes4'] = $mes_filtro;
if (!empty($data_inicio) && !empty($data_fim)) {
    $params_creditos_outros[':data_inicio4'] = $data_inicio;
    $params_creditos_outros[':data_fim4'] = $data_fim;
}

// Executar
$stmt_debitos->execute($params_debitos);
$debitos = $stmt_debitos->fetchAll(PDO::FETCH_ASSOC);

$stmt_debitos_outros->execute($params_debitos_outros);
$debitos_outros = $stmt_debitos_outros->fetchAll(PDO::FETCH_ASSOC);

$stmt_creditos->execute($params_creditos);
$creditos = $stmt_creditos->fetchAll(PDO::FETCH_ASSOC);

$stmt_creditos_outros->execute($params_creditos_outros);
$creditos_outros = $stmt_creditos_outros->fetchAll(PDO::FETCH_ASSOC);

// Combinar débitos e créditos
$todos_debitos = array_merge($debitos, $debitos_outros);
$todos_creditos = array_merge($creditos, $creditos_outros);

// ==============================================
// COMBINAR E ORDENAR LANÇAMENTOS POR DATA
// ==============================================
$lancamentos = array_merge($todos_debitos, $todos_creditos);
...
foreach ($todos_debitos as $debito) {
    ...
    if ($mes >= 1 && $mes <= 12) {
        ...
        $resumo_mensal[$mes]['status'] = 'pendente';
    }
}
                    ...
                    <table class="table table-bordered extrato-table">
                        ...
                        <tbody>
                            ...
                            <tr class="<?php echo $classe_row; ?>">
                                ...
                                <td class="text-center"><?php echo getStatusExtratoLabel($lancamento['status']); ?></td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
