...
// Buscar TODOS os anos letivos disponíveis
...
// ==============================================
$debug_info = [];
$debug_info['ano_letivo_selecionado'] = $ano_letivo;
$debug_info['aluno_id'] = $aluno_id;
$debug_info['escola_id'] = $escola_id;
$debug_info['total_chamadas_encontradas'] = count($chamadas);

// Verificar se existem dados na tabela chamada para este aluno
$sql_verifica = "SELECT COUNT(*) as total FROM chamada WHERE estudante_id = :aluno_id AND escola_id = :escola_id";
$stmt_verifica = $conn->prepare($sql_verifica);
$stmt_verifica->execute([':aluno_id' => $aluno_id, ':escola_id' => $escola_id]);
$total_geral = $stmt_verifica->fetch(PDO::FETCH_ASSOC);
$debug_info['total_chamadas_sem_filtro'] = $total_geral['total'];

// Verificar por ano
$sql_verifica_ano = "SELECT COUNT(*) as total FROM chamada WHERE estudante_id = :aluno_id AND escola_id = :escola_id AND ano_letivo_id = :ano";
$stmt_verifica_ano = $conn->prepare($sql_verifica_ano);
$stmt_verifica_ano->execute([':aluno_id' => $aluno_id, ':escola_id' => $escola_id, ':ano' => $ano_letivo]);
$total_ano = $stmt_verifica_ano->fetch(PDO::FETCH_ASSOC);
$debug_info['total_chamadas_ano_' . $ano_letivo] = $total_ano['total'];

// Verificar anos disponíveis na tabela chamada
$sql_anos_chamada = "SELECT DISTINCT ano_letivo_id, COUNT(*) as total FROM chamada WHERE estudante_id = :aluno_id GROUP BY ano_letivo_id";
$stmt_anos_chamada = $conn->prepare($sql_anos_chamada);
$stmt_anos_chamada->execute([':aluno_id' => $aluno_id]);
$anos_chamada = $stmt_anos_chamada->fetchAll(PDO::FETCH_ASSOC);
$debug_info['anos_disponiveis_chamada'] = $anos_chamada;

// Verificar estrutura da tabela
$sql_colunas = "DESCRIBE chamada";
$stmt_colunas = $conn->prepare($sql_colunas);
$stmt_colunas->execute();
$colunas = $stmt_colunas->fetchAll(PDO::FETCH_COLUMN);
$debug_info['colunas_tabela_chamada'] = $colunas;

// Ver exemplo de um registro
$sql_exemplo = "SELECT * FROM chamada WHERE estudante_id = :aluno_id LIMIT 1";
$stmt_exemplo = $conn->prepare($sql_exemplo);
$stmt_exemplo->execute([':aluno_id' => $aluno_id]);
$exemplo_registro = $stmt_exemplo->fetch(PDO::FETCH_ASSOC);
$debug_info['exemplo_registro'] = $exemplo_registro;

$mostrar_debug = (count($chamadas) == 0);
// ==============================================
...
<head>
    ...
    <style>
        ...
        .debug-box { background: #f8f9fa; border: 1px solid #dee2e6; border-radius: 8px; padding: 15px; margin-top: 20px; text-align: left; font-size: 12px; font-family: monospace; }
        .debug-box pre { background: #fff; padding: 10px; border-radius: 5px; overflow-x: auto; margin: 10px 0; }
    </style>
</head>
<body>
...
<div class="main-content-aluno">
    ...
    <div class="card border-0 shadow-sm fade-in">
        <div class="card-header bg-white fw-bold"><i class="fas fa-table"></i> Frequência por Disciplina - <?php echo $ano_letivo; ?>
            ...
            <?php if (empty($frequencias_por_disciplina)): ?>
                <div class="alert alert-info text-center">
                    ...
                    <div class="debug-box">
                        ...
                        <hr>
                        <p><strong>Ano letivo selecionado:</strong> <?php echo $debug_info['ano_letivo_selecionado']; ?></p>
                        <p><strong>Aluno ID:</strong> <?php echo $debug_info['aluno_id']; ?></p>
                        <p><strong>Escola ID:</strong> <?php echo $debug_info['escola_id']; ?></p>
                        <p><strong>Total de chamadas sem filtro:</strong> <?php echo $debug_info['total_chamadas_sem_filtro']; ?></p>
                        <p><strong>Total de chamadas para ano <?php echo $ano_letivo; ?>:</strong> <?php echo $debug_info['total_chamadas_ano_' . $ano_letivo]; ?></p>
                        
                        <p><strong>Anos disponíveis na tabela chamada:</strong></p>
                        <pre><?php print_r($debug_info['anos_disponiveis_chamada']); ?></pre>
                        
                        <p><strong>Colunas da tabela chamada:</strong> <?php echo implode(', ', $debug_info['colunas_tabela_chamada']); ?></p>
                        
                        <p><strong>Exemplo de registro (primeiro encontrado):</strong></p>
                        <pre><?php print_r($debug_info['exemplo_registro']); ?></pre>
                        
                        <?php if ($debug_info['total_chamadas_ano_' . $ano_letivo] > 0): ?>
                        <div class="alert alert-danger mt-2">
                            <strong>⚠️ Problema detectado:</strong> Existem <?php echo $debug_info['total_chamadas_ano_' . $ano_letivo]; ?> registros na tabela, mas a query não os retornou.<br>
                            Verifique se a coluna <code>ano_letivo_id</code> está com o valor correto (deve ser <?php echo $ano_letivo; ?>).
                        </div>
                        <?php endif; ?>
                    </div>
                    <?php endif; ?>
    ...
    <div class="card border-0 shadow-sm mt-4 fade-in">
        ...
        <div class="card-body"><canvas id="graficoFrequencia" height="250"></canvas></div>
    </div>
    <?php endif; ?>
...
<script>
    ...
    if (disciplinas.length > 0) {
        new Chart(document.getElementById('graficoFrequencia'), {
            ...
            options: { responsive: true, maintainAspectRatio: true, scales: { y: { beginAtZero: true, max: 100, ticks: { callback: function(value) { return value + '%'; } }, title: { display: true, text: 'Percentual (%)' } }, x: { ticks: { autoSkip: false, rotation: 45, font: { size: 10 } } } }, plugins: { tooltip: { callbacks: { label: function(context) { return 'Frequência: ' + context.raw.toFixed(1) + '%'; } } }, legend: { display: false } } }
        });
    }
</script>
</body>
