...
// Buscar disciplinas do aluno baseado na turma atual
...
// Versão simplificada - Buscar professores da turma do aluno
$sql_professores = "SELECT DISTINCT p.id, p.nome, d.id as disciplina_id, d.nome as disciplina_nome
                    FROM funcionarios p
                    JOIN professor_disciplina_turma pdt ON pdt.professor_id = p.id
                    JOIN disciplinas d ON d.id = pdt.disciplina_id
                    WHERE pdt.turma_id = (SELECT turma_id FROM matriculas WHERE estudante_id = :aluno_id AND status = 'ativa' LIMIT 1)
