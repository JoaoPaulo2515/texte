<?php
// escola/financeiro/contas_pagar/pagar.php - Registrar Pagamento
...
// Buscar conta
$stmt = $conn->prepare("
    SELECT c.*, f.nome as forma_pagamento_nome
    FROM escola_contas_pagar c
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $novo_status = ($novo_valor_pago >= $conta['valor']) ? 'pago' : 'parcial';
    
    // Registrar na tabela de transações bancárias
    $stmt_trans = $conn->prepare("
        ...
        VALUES 
        (:escola_id, NULL, 'debito', :valor, :descricao, 'conta_pagar', :data_transacao, 'confirmado', :usuario_id)
    ");
    $stmt_trans->execute([
        ':escola_id' => $escola_id,
        ':valor' => $valor_pago,
        ':descricao' => "Pagamento de conta - " . $conta['fornecedor'] . " - " . $conta['descricao'],
    ...
    $stmt->execute([
        ...
        ':juros' => $juros,
        ':status' => $novo_status,
        ':observacoes' => "Pagamento em " . date('d/m/Y') . ": " . number_format($valor_pago, 2, ',', '.') . " Kz - " . $observacoes
    ]);
...
<head>
    ...
    <title>Registrar Pagamento | SIGE Angola</title>
...
<body>
    ...
    <div class="main-content">
        <div class="top-bar"><h2><i class="fas fa-money-bill"></i> Registrar Pagamento</h2><a href="index.php" class="btn btn-secondary btn-sm"><i class="fas fa-arrow-left"></i> Voltar</a></div>
        ...
        <div class="info-box">
            <div class="row">
                <div class="col-md-6">
                    ...
                    <p><strong>Documento:</strong> <?php echo $conta['documento'] ?? '-'; ?></p>
                </div>
                <div class="col-md-6 text-end">
                    <p><strong>Valor Total:</strong> <?php echo number_format($conta['valor'], 2, ',', '.'); ?> Kz</p>
                    <p><strong>Valor já Pago:</strong> <?php echo number_format($conta['valor_pago'], 2, ',', '.'); ?> Kz</p>
        ...
        <div class="card">
            <div class="card-header bg-success text-white"><i class="fas fa-money-bill"></i> Dados do Pagamento</div>
            <div class="card-body">
                <form method="POST">
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label class="required">Valor a Pagar (Kz)</label>
                            <input type="number" step="0.01" name="valor_pago" class="form-control" value="<?php echo $saldo; ?>" required>
                    ...
                    <div class="text-center">
                        <button type="submit" class="btn btn-success"><i class="fas fa-check"></i> Registrar Pagamento</button>
