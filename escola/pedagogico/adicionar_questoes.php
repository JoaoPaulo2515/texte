...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action == 'salvar_questao') {
        ...
        $correcao_automatica = isset($_POST['correcao_automatica']) ? 1 : 0;
        $porcentagem_acerto = (int)($_POST['porcentagem_acerto'] ?? 70);
        $imagem = $_POST['imagem'] ?? '';
        ...
        if (empty($enunciado)) {
            ...
            try {
                ...
                if ($questao_id > 0) {
                    ...
                    $stmt->bindParam(':correcao_automatica', $correcao_automatica);
                    $stmt->bindParam(':porcentagem_acerto', $porcentagem_acerto);
                    $stmt->bindParam(':id', $questao_id);
                    $stmt->bindParam(':prova_id', $prova_id);
                    $stmt->execute();
                    
                    // Remover alternativas antigas (se houver)
                    ...
                    $sql = "INSERT INTO online_provas_questoes 
                            (prova_id, enunciado, tipo, pontuacao, imagem, video_url, dica, gabarito, palavras_chave, correcao_automatica, porcentagem_acerto, ordem) 
                            VALUES (:prova_id, :enunciado, :tipo, :pontuacao, :imagem, :video_url, :dica, :gabarito, :palavras_chave, :correcao_automatica, :porcentagem_acerto, :ordem)";
                    $stmt = $conn->prepare($sql);
                    $stmt->bindParam(':prova_id', $prova_id);
                    $stmt->bindParam(':enunciado', $enunciado);
                    $stmt->bindParam(':tipo', $tipo);
                ...
                // SALVAR ALTERNATIVAS (apenas para múltipla escolha e verdadeiro/falso)
                ...
                // Para questões dissertativas, não há alternativas para salvar
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .palavras-chave-container .form-label {
            color: #1565c0;
        }
   ...
   <?php include __DIR__ . '/includes/menu_pedagogico.php'; ?>
                ...
                <div class="form-card-body">
                    <form method="POST" id="formQuestao" onsubmit="return antesDeEnviar()">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="mb-3">
                                    <label class="form-label"><i class="fas fa-image"></i> Imagem (URL)</label>
                                    <input type="text" name="imagem" class="form-control" placeholder="https://..." value="<?php echo $questao_edit['imagem'] ?? ''; ?>">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label class="form-label"><i class="fas fa-video"></i> Vídeo (URL)</label>
                                    <input type="text" name="video_url" class="form-control" placeholder="https://youtu.be/..." value="<?php echo $questao_edit['video_url'] ?? ''; ?>">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <label class="form-label"><i class="fas fa-lightbulb"></i> Dica para o Aluno</label>
                                    <input type="text" name="dica" class="form-control" placeholder="Dica para ajudar na resolução..." value="<?php echo $questao_edit['dica'] ?? ''; ?>">
                                </div>
                            </div>
                        </div>
                        
                        <!-- Alternativas (Múltipla Escolha e V/F) -->
                        ...
                        <div id="gabarito_div" style="display: none;">
                            <div class="gabarito-container">
                                <label class="form-label"><i class="fas fa-check-double text-success"></i> Gabarito (Resposta Esperada)</label>
                                <textarea name="gabarito" class="form-control" rows="4" placeholder="Digite a resposta esperada para esta questão..."><?php echo htmlspecialchars($questao_edit['gabarito'] ?? ''); ?></textarea>
                                <small class="text-muted mt-1 d-block">Esta será a referência para correção da questão dissertativa</small>
                            </div>
                            
                            <div class="palavras-chave-container">
                                <label class="form-label"><i class="fas fa-key text-primary"></i> Palavras-chave (opcional)</label>
                                <input type="text" name="palavras_chave" class="form-control" placeholder="Ex: matemática, álgebra, equação (separar por vírgula)" value="<?php echo htmlspecialchars($questao_edit['palavras_chave'] ?? ''); ?>">
                                <small class="text-muted mt-1 d-block">Palavras que devem estar presentes na resposta do aluno</small>
                                <div class="mt-2" id="preview_palavras">
                                    <?php if (!empty($questao_edit['palavras_chave'])): ?>
                                        <?php foreach(explode(',', $questao_edit['palavras_chave']) as $palavra): ?>
                                            <span class="badge-palavra me-1 mb-1"><i class="fas fa-tag"></i> <?php echo trim(htmlspecialchars($palavra)); ?></span>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
                                </div>
                            </div>
                            ...
                            <div class="row mt-3">
                                <div class="col-md-6">
                                    <div class="form-check">
                                        <input type="checkbox" class="form-check-input" id="correcao_automatica" name="correcao_automatica" value="1" <?php echo ($questao_edit['correcao_automatica'] ?? 0) ? 'checked' : ''; ?>>
                                        <label class="form-check-label" for="correcao_automatica">
                                            <i class="fas fa-robot"></i> Corrigir automaticamente (sugestão)
                                        </label>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">Porcentagem de acerto para aprovação</label>
                                    <input type="number" name="porcentagem_acerto" class="form-control" value="<?php echo $questao_edit['porcentagem_acerto'] ?? 70; ?>" min="0" max="100">
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="questao-item" data-id="<?php echo $q['id']; ?>">
                                ...
                                <div class="questao-body">
                                    ...
                                    <?php elseif ($q['tipo'] == 'dissertativa'): ?>
                                        ...
                                        <?php if ($q['gabarito']): ?>
                                            <div class="gabarito-preview mt-2">
                                                <strong><i class="fas fa-check-double text-success"></i> Gabarito:</strong>
                                                <p class="mb-0 small"><?php echo htmlspecialchars(substr($q['gabarito'], 0, 100)) . (strlen($q['gabarito']) > 100 ? '...' : ''); ?></p>
                                            </div>
                                        <?php endif; ?>
                                        <?php if ($q['palavras_chave']): ?>
                                            <div class="mt-2">
                                                <?php foreach(explode(',', $q['palavras_chave']) as $palavra): ?>
                                                    <span class="badge-palavra me-1"><i class="fas fa-tag"></i> <?php echo trim(htmlspecialchars($palavra)); ?></span>
                                                <?php endforeach; ?>
                                            </div>
                                        <?php endif; ?>
                                    <?php endif; ?>
    ...
    <script>
        ...
        function mudarTipoQuestao() {
            ...
            const divGabarito = document.getElementById('gabarito_div');
            const previewPalavras = document.getElementById('preview_palavras');
            const inputPalavras = document.querySelector('input[name="palavras_chave"]');
            ...
            if (tipo == 'multipla_escolha') {
                ...
                divGabarito.style.display = 'block';
            }
            
            // Preview das palavras-chave
            if (inputPalavras) {
                inputPalavras.addEventListener('input', function() {
                    let palavras = this.value.split(',');
                    let html = '';
                    palavras.forEach(palavra => {
                        if (palavra.trim()) {
                            html += `<span class="badge-palavra me-1 mb-1"><i class="fas fa-tag"></i> ${palavra.trim()}</span>`;
                        }
                    });
                    if (previewPalavras) previewPalavras.innerHTML = html;
                });
            }
        }
