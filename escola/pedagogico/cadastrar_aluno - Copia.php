...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cadastrar_aluno'])) {
    ...
    try {
        ...
        if (!empty($encarregado_telefone)) {
            ...
            $mensagem .= "Seu educando(a) *{$nome_completo}* foi cadastrado(a) com sucesso.\n\n";
            $mensagem .= "📋 Nº Processo: *{$numero_processo}*\n";
            $mensagem .= "🔐 Usuário: *{$usuario_acesso}*\n";
            $mensagem .= "🔐 Senha: *{$senha_acesso}*\n\n";
            $mensagem .= "🌐 Acesse: " . $_SERVER['REQUEST_SCHEME'] . "://" . $_SERVER['HTTP_HOST'] . "/sige_Plataforma/escola/aluno/login.php\n\n";
            $resultado_whatsapp = enviarWhatsAppEscola($encarregado_telefone, $mensagem, $whatsapp_escola);
            $success .= "<br><br><a href='{$resultado_whatsapp['url']}' target='_blank' class='btn btn-success'><i class='fab fa-whatsapp'></i> Enviar Credenciais via WhatsApp</a>
                         <button onclick='copiarCredenciais()' class='btn btn-secondary ms-2'><i class='fas fa-copy'></i> Copiar Credenciais</button>";
        }
