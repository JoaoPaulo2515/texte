<?php
// escola/professor/financeiro.php - Gestão Financeira do Professor
...
// Tabela de salários
$sql_salarios = "
CREATE TABLE IF NOT EXISTS `professor_salarios` (
...
$sql_vales = "
CREATE TABLE IF NOT EXISTS `professor_vales` (
    `id` INT NOT NULL AUTO_INCREMENT,
    `professor_id` INT NOT NULL,
...
// Tabela de férias
$sql_ferias = "
CREATE TABLE IF NOT EXISTS `professor_ferias` (
    `id` INT NOT NULL AUTO_INCREMENT,
    `professor_id` INT NOT NULL,
    ...
    PRIMARY KEY (`id`),
    UNIQUE KEY `unique_professor_ano` (`professor_id`, `ano_referente`),
    KEY `idx_professor_id` (`professor_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci
";
...
CREATE TABLE IF NOT EXISTS `professor_solicitacoes_ferias` (
    `id` INT NOT NULL AUTO_INCREMENT,
    `professor_id` INT NOT NULL,
...
// Tabela de despesas
$sql_despesas = "
CREATE TABLE IF NOT EXISTS `professor_despesas` (
    `id` INT NOT NULL AUTO_INCREMENT,
    `professor_id` INT NOT NULL,
...
$sql_rendimentos = "
CREATE TABLE IF NOT EXISTS `professor_rendimentos` (
    `id` INT NOT NULL AUTO_INCREMENT,
    `professor_id` INT NOT NULL,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'solicitar_vale') {
    ...
    if ($valor <= 0) {
        ...
        $sql = "INSERT INTO professor_vales (professor_id, valor, motivo, data_prevista_devolucao, status) 
                VALUES (:professor_id, :valor, :motivo, :data_prevista_devolucao, 'pendente')";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'solicitar_ferias') {
    ...
    $ano_atual = date('Y');
    $sql_ferias = "SELECT dias_restantes FROM professor_ferias WHERE professor_id = :professor_id AND ano_referente = :ano";
    $stmt_ferias = $conn->prepare($sql_ferias);
    $stmt_ferias->execute([':professor_id' => $professor_id, ':ano' => $ano_atual]);
    $ferias = $stmt_ferias->fetch(PDO::FETCH_ASSOC);
    
    if (!$ferias) {
        // Criar registro de férias para o ano
        $sql_insert = "INSERT INTO professor_ferias (professor_id, ano_referente, dias_totais, dias_usados, dias_restantes) 
                       VALUES (:professor_id, :ano, 30, 0, 30)";
        $stmt_insert = $conn->prepare($sql_insert);
        $stmt_insert->execute([':professor_id' => $professor_id, ':ano' => $ano_atual]);
        $dias_restantes = 30;
    } else {
    ...
    if ($dias_solicitados > $dias_restantes) {
        ...
        $sql = "INSERT INTO professor_solicitacoes_ferias (professor_id, data_inicio, data_fim, dias_solicitados, motivo, status) 
                VALUES (:professor_id, :data_inicio, :data_fim, :dias_solicitados, :motivo, 'pendente')";
        $stmt = $conn->prepare($sql);
        $stmt->execute([
            ':professor_id' => $professor_id,
        ...
        $mensagem = "Solicitação de férias enviada com sucesso! Aguardando aprovação.";
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'registrar_despesa') {
    ...
    if ($valor <= 0) {
        ...
        $sql = "INSERT INTO professor_despesas (professor_id, categoria, descricao, valor, data_despesa) 
                VALUES (:professor_id, :categoria, :descricao, :valor, :data_despesa)";
        $stmt = $conn->prepare($sql);
        $stmt->execute([
            ':professor_id' => $professor_id,
            ':categoria' => $categoria,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'registrar_rendimento') {
    ...
    if ($valor <= 0) {
        ...
        $sql = "INSERT INTO professor_rendimentos (professor_id, tipo, descricao, valor, data_rendimento) 
                VALUES (:professor_id, :tipo, :descricao, :valor, :data_rendimento)";
        $stmt = $conn->prepare($sql);
        $stmt->execute([
            ':professor_id' => $professor_id,
            ':tipo' => $tipo,
...
$sql_salarios_lista = "SELECT * FROM professor_salarios WHERE professor_id = :professor_id ORDER BY ano DESC, mes DESC";
$stmt_salarios = $conn->prepare($sql_salarios_lista);
$stmt_salarios->execute([':professor_id' => $professor_id]);
$salarios = $stmt_salarios->fetchAll(PDO::FETCH_ASSOC);

// Vales solicitados
$sql_vales_lista = "SELECT * FROM professor_vales WHERE professor_id = :professor_id ORDER BY created_at DESC";
$stmt_vales = $conn->prepare($sql_vales_lista);
$stmt_vales->execute([':professor_id' => $professor_id]);
$vales = $stmt_vales->fetchAll(PDO::FETCH_ASSOC);

// Solicitações de férias
$sql_solicitacoes = "SELECT * FROM professor_solicitacoes_ferias WHERE professor_id = :professor_id ORDER BY created_at DESC";
$stmt_solicitacoes = $conn->prepare($sql_solicitacoes);
$stmt_solicitacoes->execute([':professor_id' => $professor_id]);
$solicitacoes_ferias = $stmt_solicitacoes->fetchAll(PDO::FETCH_ASSOC);

// Informações de férias do ano atual
$ano_atual = date('Y');
$sql_ferias_info = "SELECT * FROM professor_ferias WHERE professor_id = :professor_id AND ano_referente = :ano";
$stmt_ferias_info = $conn->prepare($sql_ferias_info);
$stmt_ferias_info->execute([':professor_id' => $professor_id, ':ano' => $ano_atual]);
...
// Despesas
$sql_despesas_lista = "SELECT * FROM professor_despesas WHERE professor_id = :professor_id ORDER BY data_despesa DESC LIMIT 10";
$stmt_despesas = $conn->prepare($sql_despesas_lista);
$stmt_despesas->execute([':professor_id' => $professor_id]);
$despesas = $stmt_despesas->fetchAll(PDO::FETCH_ASSOC);

// Total de despesas
$sql_total_despesas = "SELECT SUM(valor) as total FROM professor_despesas WHERE professor_id = :professor_id AND status = 'pendente'";
$stmt_total_despesas = $conn->prepare($sql_total_despesas);
$stmt_total_despesas->execute([':professor_id' => $professor_id]);
$total_despesas = $stmt_total_despesas->fetch(PDO::FETCH_ASSOC)['total'] ?? 0;

// Rendimentos extras
$sql_rendimentos_lista = "SELECT * FROM professor_rendimentos WHERE professor_id = :professor_id ORDER BY data_rendimento DESC LIMIT 10";
$stmt_rendimentos = $conn->prepare($sql_rendimentos_lista);
$stmt_rendimentos->execute([':professor_id' => $professor_id]);
$rendimentos = $stmt_rendimentos->fetchAll(PDO::FETCH_ASSOC);

// Total de rendimentos no mês
$mes_atual = date('m');
$sql_total_rendimentos_mes = "SELECT SUM(valor) as total FROM professor_rendimentos WHERE professor_id = :professor_id AND MONTH(data_rendimento) = :mes";
$stmt_total_rendimentos = $conn->prepare($sql_total_rendimentos_mes);
$stmt_total_rendimentos->execute([':professor_id' => $professor_id, ':mes' => $mes_atual]);
$total_rendimentos_mes = $stmt_total_rendimentos->fetch(PDO::FETCH_ASSOC)['total'] ?? 0;

// Último salário
$sql_ultimo_salario = "SELECT * FROM professor_salarios WHERE professor_id = :professor_id ORDER BY ano DESC, mes DESC LIMIT 1";
$stmt_ultimo = $conn->prepare($sql_ultimo_salario);
$stmt_ultimo->execute([':professor_id' => $professor_id]);
...
<head>
    ...
    <title>Financeiro - SIGE Angola</title>
...
<div class="container">
    <div class="header">
        <div>
            <h1><i class="fas fa-coins"></i> Gestão Financeira</h1>
            <p>Salários, vales, férias e despesas</p>
        </div>
                    ...
                    <table>
                        ...
                        <tbody>
                            <?php foreach ($salarios as $s): ?>
                                <tr>
                                    <td><strong><?php echo $nome_meses[$s['mes']] . '/' . $s['ano']; ?></strong></td>
                                    ...
                                    <td>
                                        <span class="badge-status <?php echo $s['status'] == 'pago' ? 'badge-aprovado' : ($s['status'] == 'pendente' ? 'badge-pendente' : ''); ?>">
                                            <?php echo $s['status'] == 'pago' ? 'Pago' : ($s['status'] == 'pendente' ? 'Pendente' : 'Atrasado'); ?>
                                        </span>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
    ...
    <div class="row">
        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-header">
                    <i class="fas fa-hand-holding-usd"></i> Vales Solicitados
                    <button class="btn-primary-custom float-end" style="padding: 5px 15px; font-size: 12px;" onclick="abrirModalVale()">
                ...
                <div class="card-body">
                    ...
                    <?php else: ?>
                        <div class="table-responsive">
                            <table>
                                <thead>
                                    <tr><th>Data</th><th>Valor</th><th>Motivo</th><th>Status</th></tr>
                                </thead>
                                <tbody>
                                    <?php foreach ($vales as $v): ?>
                                        <tr>
                                            <td><?php echo date('d/m/Y', strtotime($v['data_solicitacao'])); ?></td>
                                            <td><?php echo number_format($v['valor'], 2); ?> Kz</td>
                                            <td><small><?php echo htmlspecialchars(substr($v['motivo'], 0, 30)) . (strlen($v['motivo']) > 30 ? '...' : ''); ?></small></td>
                                            <td><span class="badge-status <?php echo $v['status'] == 'aprovado' ? 'badge-aprovado' : ($v['status'] == 'pendente' ? 'badge-pendente' : 'badge-rejeitado'); ?>"><?php echo $v['status']; ?></span></td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-header">
                    <i class="fas fa-umbrella-beach"></i> Plano de Férias
                    <button class="btn-primary-custom float-end" style="padding: 5px 15px; font-size: 12px;" onclick="abrirModalFerias()">
                    ...
                    <?php if (!empty($solicitacoes_ferias)): ?>
                        <div class="table-responsive">
                            <table class="table table-sm">
                                ...
                                <tbody>
                                    <?php foreach ($solicitacoes_ferias as $sf): ?>
                                        <tr>
                                            <td><?php echo date('d/m/Y', strtotime($sf['data_inicio'])); ?> - <?php echo date('d/m/Y', strtotime($sf['data_fim'])); ?></td>
                                            <td><?php echo $sf['dias_solicitados']; ?> dias</td>
                                            <td><span class="badge-status <?php echo $sf['status'] == 'aprovado' ? 'badge-aprovado' : ($sf['status'] == 'pendente' ? 'badge-pendente' : 'badge-rejeitado'); ?>"><?php echo $sf['status']; ?></span></td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
    ...
    <div class="row">
        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-header">
                    <i class="fas fa-receipt"></i> Despesas Pendentes
                    <button class="btn-primary-custom float-end" style="padding: 5px 15px; font-size: 12px;" onclick="abrirModalDespesa()">
                        <i class="fas fa-plus"></i> Nova Despesa
                    </button>
                </div>
                <div class="card-body">
                    <div class="alert alert-info">
                        <strong>Total pendente:</strong> <?php echo number_format($total_despesas, 2); ?> Kz
                    </div>
                    ...
                    <?php else: ?>
                        <div class="table-responsive">
                            <table class="table table-sm">
                                <thead><tr><th>Data</th><th>Categoria</th><th>Descrição</th><th>Valor</th><th>Status</th></tr></thead>
                                <tbody>
                                    <?php foreach ($despesas as $d): ?>
                                        <tr>
                                            <td><?php echo date('d/m/Y', strtotime($d['data_despesa'])); ?></td>
                                            <td><?php echo $d['categoria']; ?></td>
                                            <td><small><?php echo htmlspecialchars(substr($d['descricao'], 0, 25)); ?></small></td>
                                            <td><?php echo number_format($d['valor'], 2); ?> Kz</td>
                                            <td><span class="badge-status badge-pendente"><?php echo $d['status']; ?></span></td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-header">
                    ...
                    <button class="btn-primary-custom float-end" style="padding: 5px 15px; font-size: 12px;" onclick="abrirModalRendimento()">
                ...
                <div class="card-body">
                    <?php if (empty($rendimentos)): ?>
                        <div class="text-center text-muted py-3">
                            ...
                            <p>Nenhum rendimento extra registrado.</p>
                        </div>
                    <?php else: ?>
                        <div class="table-responsive">
                            <table class="table table-sm">
                                ...
                                <tbody>
                                    <?php foreach ($rendimentos as $r): ?>
                                        <tr>
                                            <td><?php echo date('d/m/Y', strtotime($r['data_rendimento'])); ?></td>
                                            <td><?php echo $r['tipo']; ?></td>
                                            <td><small><?php echo htmlspecialchars(substr($r['descricao'], 0, 25)); ?></small></td>
                                            <td class="text-success">+ <?php echo number_format($r['valor'], 2); ?> Kz</td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
        ...
        <div class="modal-custom-body">
            <form method="POST">
                ...
                <div class="form-group">
                    ...
                    <textarea name="motivo" class="form-control" rows="3" required placeholder="Informe o motivo da solicitação"></textarea>
                </div>
                <div class="form-group">
    ...
    <div class="modal-custom-content">
        ...
        <div class="modal-custom-body">
            <form method="POST">
                ...
                <div class="alert alert-info">
                    <i class="fas fa-info-circle"></i> Dias disponíveis: <?php echo $ferias_info['dias_restantes']; ?>
                </div>
                <div class="text-end">
    ...
    <div class="modal-custom-content">
        ...
        <div class="modal-custom-body">
            <form method="POST">
                ...
                <div class="form-group">
                    <label class="form-label">Valor (Kz)</label>
                    <input type="text" name="valor" class="form-control" required placeholder="Ex: 25000,00">
                </div>
                <div class="form-group">
...
<div id="modalRendimento" class="modal-custom">
    <div class="modal-custom-content">
        <div class="modal-custom-header modal-success">
            <h3><i class="fas fa-chart-line"></i> Registrar Rendimento Extra</h3>
...
<script>
    ...
    function abrirModalVale() { abrirModal('modalVale'); }
    function abrirModalFerias() { abrirModal('modalFerias'); }
    function abrirModalDespesa() { abrirModal('modalDespesa'); }
    function abrirModalRendimento() { abrirModal('modalRendimento'); }
