...
// Se não tiver ano letivo selecionado, buscar o mais recente
if ($ano_letivo_id == 0) {
    $sql_ultimo_ano = "SELECT id FROM ano_letivo ORDER BY ano DESC LIMIT 1";
    $stmt_ultimo = $conn->prepare($sql_ultimo_ano);
    $stmt_ultimo->execute();
    $ultimo = $stmt_ultimo->fetch(PDO::FETCH_ASSOC);
    if ($ultimo) {
        $ano_letivo_id = $ultimo['id'];
    }
}

// Buscar ano letivo atual para exibir
$sql_ano_atual = "SELECT id, ano FROM ano_letivo WHERE id = :id";
$stmt_ano_atual = $conn->prepare($sql_ano_atual);
$stmt_ano_atual->execute([':id' => $ano_letivo_id]);
$ano_atual_info = $stmt_ano_atual->fetch(PDO::FETCH_ASSOC);
$ano_atual_texto = $ano_atual_info ? $ano_atual_info['ano'] : date('Y');
...
$sql_turmas = "
    ...
    FROM turmas t
    LEFT JOIN matriculas m ON m.turma_id = t.id AND m.ano_letivo = :ano_letivo_id AND m.status = 'ativa'
    LEFT JOIN disciplina_turma dt ON dt.turma_id = t.id
    LEFT JOIN disciplinas d ON d.id = dt.disciplina_id
...
$turmas_ativas = count(array_filter($turmas, function($t) { return $t['status'] == 'ativa'; }));
...
<div class="container">
    ...
    <div class="filtros">
        <div class="filtro-group">
            ...
            <select id="anoLetivoId" class="filtro-select" onchange="filtrarAno()">
    ...
    <div class="card">
        <div class="card-header">
            ...
            <span style="font-size: 12px;">Total: <?php echo $total_turmas; ?> turmas | Ano Letivo: <?php echo htmlspecialchars($ano_atual_texto); ?></span>
        </div>
        <div class="card-body">
            <?php if (empty($turmas)): ?>
                    ...
                    <tbody>
                        <?php foreach ($turmas as $turma): ?>
                            <tr>
                                ...
                                <td>
                                    <span class="badge <?php echo $turma['status'] == 'ativa' ? 'badge-ativo' : 'badge-inativo'; ?>">
                                        <?php echo $turma['status'] == 'ativa' ? 'ATIVA' : 'INATIVA'; ?>
                                    </span>
                                </td>
                                <td class="btn-acoes">
...
<script>
    ...
    function filtrarAno() {
        const anoId = document.getElementById('anoLetivoId').value;
        window.location.href = `listar_turmas.php?ano_letivo_id=${anoId}`;
    }
    ...
    fetch(`buscar_turma.php?id=${id}`)
        ...
        .then(data => {
            ...
            if (!data.success) {
                modalBody.innerHTML = `<div style="color: red; padding: 20px; text-align: center;">
                    ❌ Erro ao carregar dados da turma.<br>
                    <button onclick="fecharModal()" style="margin-top: 10px; padding: 5px 15px;">Fechar</button>
                </div>`;
                return;
            }
            ...
            let html = `
                ...
                <div style="background: #f8f9fa; padding: 10px; border-radius: 8px; margin-bottom: 15px;">
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 10px;">
                        <div><strong>📚 Ano:</strong> ${data.ano}ª Classe</div>
                        <div><strong>🕐 Turno:</strong> ${escapeHtml(data.turno)}</div>
                        <div><strong>🏠 Sala:</strong> ${escapeHtml(data.sala)}</div>
                        <div><strong>📅 Ano Letivo:</strong> ${escapeHtml(data.ano_letivo)}</div>
                        <div><strong>🎓 Curso:</strong> ${escapeHtml(data.curso)}</div>
                        <div><strong>📖 Classe:</strong> ${escapeHtml(data.classe)}</div>
                        <div><strong>⏰ Horário:</strong> ${escapeHtml(data.horario)}</div>
                        <div><strong>📊 Status:</strong> <span class="badge ${data.status == 'ativa' ? 'badge-ativo' : 'badge-inativo'}">${data.status.toUpperCase()}</span></div>
                    </div>
                </div>
                
                <!-- Estatísticas -->
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 10px; margin-bottom: 15px;">
                    <div style="background: #d4e6f1; padding: 10px; border-radius: 8px; text-align: center;">
                        <div style="font-size: 20px; font-weight: bold;">${data.total_alunos}</div>
                        <div style="font-size: 11px;">Alunos Matriculados</div>
                    </div>
                    <div style="background: #d5f4e6; padding: 10px; border-radius: 8px; text-align: center;">
                        <div style="font-size: 20px; font-weight: bold;">${data.capacidade}</div>
                        <div style="font-size: 11px;">Capacidade Máxima</div>
                    </div>
                    <div style="background: #fef9e7; padding: 10px; border-radius: 8px; text-align: center;">
                        <div style="font-size: 20px; font-weight: bold;">${data.percentual_ocupacao}%</div>
                        <div style="font-size: 11px;">Ocupação</div>
                    </div>
                    <div style="background: ${data.vagas_disponiveis > 0 ? '#d5f4e6' : '#fadbd8'}; padding: 10px; border-radius: 8px; text-align: center;">
                        <div style="font-size: 20px; font-weight: bold;">${data.vagas_disponiveis}</div>
                        <div style="font-size: 11px;">Vagas Disponíveis</div>
                    </div>
                </div>
                
                <!-- Disciplinas -->
                <h4 style="margin: 15px 0 10px 0; color: #1e5799;">📚 Disciplinas</h4>
                <div style="display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 15px;">
                    ${data.disciplinas && data.disciplinas.length > 0 ? 
                        data.disciplinas.map(d => `<span class="badge" style="background: #ecf0f1; color: #2c3e50; padding: 5px 12px;">📖 ${escapeHtml(d.nome)}</span>`).join('') : 
                        '<p>Nenhuma disciplina associada.</p>'}
                </div>
                
                <!-- Professores -->
                <h4 style="margin: 15px 0 10px 0; color: #1e5799;">👨‍🏫 Professores</h4>
                            ...
                            <tbody>
                                ${data.professores.map(p => `
                                    <tr>
                                        ...
                                        <td style="padding: 8px;">${escapeHtml(p.disciplina_nome)}</td>
                                        <td style="padding: 8px;">${escapeHtml(p.email || '-')} / ${escapeHtml(p.telefone || '-')}</td>
                                    </tr>
                                `).join('')}
                            </tbody>
                        </table>` : 
                ...
                <!-- Alunos -->
                <h4 style="margin: 15px 0 10px 0; color: #1e5799;">👨‍🎓 Alunos Matriculados (últimos 20)</h4>
                        ...
                        `<table style="width: 100%; border-collapse: collapse;">
                            ...
                            <tbody>
                                ${data.alunos.map(aluno => `
                                    <tr>
                                        <td style="padding: 8px;">${escapeHtml(aluno.nome)}</td>
                                        <td style="padding: 8px;">${escapeHtml(aluno.matricula)}</td>
