...
$sql_anos_letivos = "SELECT id, ano, data_inicio, data_fim, ativo FROM ano_letivo ORDER BY ano DESC";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'transferir') {
    ...
    if ($ano_letivo_id > 0) {
        $sql_check_ano = "SELECT id, ano FROM ano_letivo WHERE id = :id";
        $stmt_check_ano = $conn->prepare($sql_check_ano);
        $stmt_check_ano->execute([':id' => $ano_letivo_id]);
        if (!$stmt_check_ano->fetch()) {
            $erros[] = "Ano letivo inválido.";
        }
    }
    ...
    if (empty($erros)) {
        try {
            ...
            $sql_update = "UPDATE matriculas SET status = 'transferida', data_cancelamento = NOW() WHERE id = :matricula_id";
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-secondary:hover { transform: translateY(-2px); box-shadow: 0 5px 15px rgba(0,0,0,0.2); }
        .btn-danger { background: linear-gradient(135deg, #c0392b, #e74c3c); color: white; }
        .btn-danger:hover { transform: translateY(-2px); box-shadow: 0 5px 15px rgba(0,0,0,0.2); }
            ...
            <?php if ($aluno_selecionado): ?>
                <div class="aluno-info">
                    ...
                    <div class="aluno-details">
                        ...
                        <div class="aluno-detail-item">
                            ...
                            <span><strong>Escola:</strong> <?php echo htmlspecialchars($matricula_ativa['escola_nome'] ?? $dados_aluno['escola_nome'] ?? '---'); ?></span>
                        </div>
                        <div class="aluno-detail-item">
                ...
                <form method="POST" action="" id="formTransferencia">
                    ...
                    <div class="form-row">
                            ...
                            <select name="ano_letivo_id" class="form-control" required>
                                ...
                                <?php foreach ($anos_letivos as $ano): ?>
                                    <option value="<?php echo $ano['id']; ?>" 
                                        ...
                                        <?php if ($ano['ativo'] == 1): ?> (Ativo) <?php endif; ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            <label>Data da Transferência <span class="required">*</span></label>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            <label>Motivo da Transferência <span class="required">*</span></label>
                    ...
                    <div class="form-group">
                        <label>Escola de Destino <span class="required">*</span></label>
                    ...
                    <div class="btn-group">
                        <a href="transferir_aluno.php" class="btn btn-secondary">
                            🔄 Limpar
                        </a>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="status-badge status-<?php echo $transf['status']; ?>">
                                            <?php echo strtoupper($transf['status']); ?>
                                        </span>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
            ...
            timeoutId = setTimeout(() => {
                fetch(`buscar_alunos.php?termo=${encodeURIComponent(termo)}`)
                    ...
                    .then(data => {
                        ...
                        data.forEach(aluno => {
                            html += `
                                <div class="aluno-item" onclick="selecionarAluno(${aluno.id})">
                                    <div class="aluno-nome">${escapeHtml(aluno.nome)}</div>
                                    <div class="aluno-matricula">
                                        Matrícula: ${escapeHtml(aluno.matricula || '---')} | BI: ${escapeHtml(aluno.bi || '---')}
                                    </div>
                                </div>
                            `;
                        });
