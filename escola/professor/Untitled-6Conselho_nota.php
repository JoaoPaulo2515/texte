...
function calcularSituacaoAluno($conn, $aluno_id, $serie, $notas_por_disciplina, $escala_max, $escala_aprovacao) {
    ...
    // Buscar critérios de avaliação para a série
    $sql_criterio = "
    ...
    // Identificar disciplinas principais
    $disciplinas_principais = ['Matemática', 'Português', 'Matematica', 'Portugues', 'Língua Portuguesa'];
    ...
    foreach ($notas_por_disciplina as $disciplina_id => $disciplina_notas) {
        // Para o 3º bimestre, usar media_final, senão media_parcial do 3º bimestre
        $media_final = 0;
        if (isset($disciplina_notas[3]['media_final']) && $disciplina_notas[3]['media_final'] > 0) {
            ...
            // Buscar a maior média entre os bimestres
            $melhor_media = 0;
            for ($b = 1; $b <= 3; $b++) {
        ...
        if ($media_final > 0) {
            ...
            $count_disciplinas++;
            $medias_por_disciplina[$disciplina_id] = $media_final;
            
            $eh_negativa = ($media_final < $media_aprovacao);
            
            if ($eh_negativa) {
                $total_negativas++;
                $disciplinas_negativas[] = $disciplina_id;
                
                // Buscar nome da disciplina para verificar se é Matemática ou Português
                ...
                if ($disc_nome) {
                    ...
                    if (strpos($nome_lower, 'matem') !== false) {
                        $tem_negativa_matematica = true;
                    }
                    if (strpos($nome_lower, 'portug') !== false || strpos($nome_lower, 'língua') !== false) {
                        $tem_negativa_portugues = true;
                    }
                }
            }
        }
    ...
    if ($ehClasseExame) {
        if ($total_negativas == 0) {
            ...
            if ($pode_recurso) {
                $mensagem_situacao .= " | PODE SOLICITAR RECURSO";
            }
        }
    } 
    // Classes Normais (Ensino Fundamental e Secundário)
    else {
        if ($total_negativas == 0) {
            $situacao = 'aprovado';
        ...
        elseif ($permite_negativas && $total_negativas <= $limite_negativas) {
            // Verificar se tem Matemática e Português negativas ao mesmo tempo
            if ($proibido_matematica_portugues && $tem_negativa_matematica && $tem_negativa_portugues) {
                $situacao = 'reprovado';
                $mensagem_situacao = "❌ REPROVADO - Reprovado por ter Matemática e Português negativas simultaneamente";
                $pode_recurso = true;
                $mensagem_situacao .= " | PODE SOLICITAR RECURSO";
            } else {
                $situacao = 'aprovado_com_recurso';
                $mensagem_situacao = "🔄 APROVADO COM RECURSO - $total_negativas disciplina(s) negativa(s) (limite: $limite_negativas)";
                $pode_recurso = true;
            }
        } 
        else {
            $situacao = 'reprovado';
            $mensagem_situacao = "❌ REPROVADO - Excedeu o limite de $limite_negativas disciplina(s) negativa(s). Total: $total_negativas";
            $pode_recurso = ($criterio && $criterio['permite_recurso']);
            if ($pode_recurso) {
                $mensagem_situacao .= " | PODE SOLICITAR RECURSO";
            }
    ...
    return [
        ...
        'criterio' => $criterio,
        'disciplinas_negativas' => $disciplinas_negativas,
        'tem_negativa_matematica' => $tem_negativa_matematica,
        'tem_negativa_portugues' => $tem_negativa_portugues
    ];
}
...
<body>
                    ...
                    <div class="row align-items-center">
                        ...
                        <div class="col-md-3 text-center">
                            <div class="votos-container">
                                ...
                                <span class="votos-favoraveis"><i class="fas fa-thumbs-up me-1"></i> <?php echo $solic['votos_favoraveis']; ?> ✅</span>
                                <span style="font-size: 1.1rem; font-weight: bold;">/</span>
                                <span class="votos-contra"><i class="fas fa-thumbs-down me-1"></i> <?php echo $solic['votos_contra']; ?> ❌</span>
                            </div>
                        </div>
    ...
    <div class="modal fade" id="modalFichaAluno" tabindex="-1">
        <div class="modal-dialog modal-lg modal-ficha">
            <div class="modal-content">
                <div class="modal-header modal-header-custom">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="d-flex gap-3 flex-wrap">
                            <div class="form-check">
                                ...
                                <label class="form-check-label text-success" for="voto_favoravel"><i class="fas fa-check-circle"></i> Favorável</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="voto" id="voto_contra" value="contra">
                                <label class="form-check-label text-danger" for="voto_contra"><i class="fas fa-times-circle"></i> Contra</label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="voto" id="voto_abstencao" value="abstencao">
                                <label class="form-check-label text-secondary" for="voto_abstencao"><i class="fas fa-minus-circle"></i> Abstenção</label>
                            </div>
                        </div>
                    </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" onclick="registrarVoto()"><i class="fas fa-vote-yea"></i> Registrar Voto</button>
                </div>
            </div>
        </div>
    </div>
        ...
        function renderizarAlunos(sessaoId) {
            ...
            for (let aluno of filtered) {
                ...
                let fotoHtml = aluno.foto ? 
                    `<img src="../uploads/alunos/${aluno.foto}" style="width:45px;height:45px;border-radius:50%;object-fit:cover;cursor:pointer;" onclick="event.stopPropagation(); ampliarImagem('${aluno.foto}', '${aluno.nome.replace(/'/g, "\\'")}')">` :
                    `<div class="aluno-foto" style="width:45px;height:45px;background:#e9ecef;border-radius:50%;display:flex;align-items:center;justify-content:center;cursor:pointer;" onclick="event.stopPropagation(); ampliarImagem(null, '${aluno.nome.replace(/'/g, "\\'")}')"><i class="fas fa-user text-secondary"></i></div>`;
                
                html += `
                    <div class="aluno-card" onclick="abrirFichaAluno(${aluno.matricula_id})">
                        <div class="row align-items-center">
                            <div class="col-auto">${fotoHtml}</div>
                            <div class="col">
