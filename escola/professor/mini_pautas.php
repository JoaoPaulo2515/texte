...
function getCorNota($nota, $is_ensino_fundamental) {
    if ($nota === null || $nota === '' || $nota <= 0) {
        return ''; // Sem cor para notas vazias
    }
    
    if ($is_ensino_fundamental) {
        // Ensino Fundamental (1ª à 6ª Classe) - Escala 0-10
        if ($nota >= 4.5) {
            return 'cor-verde'; // Verde para notas positivas
        } else {
            return 'cor-vermelha'; // Vermelho para notas negativas
        }
    } else {
...
function getNotaClass($nota, $is_ensino_fundamental) {
    ...
    elseif ($nota == $limite && $nota > 0) return 'nota-regular';
...
<body>
    ...
    <div class="main-content">
        ...
        <!-- Info de Regras do 3º Bimestre -->
        <?php if ($bimestre == 3): ?>
        <div class="alert alert-info mb-3 fade-in">
            <i class="fas fa-info-circle"></i> 
            <strong>Regras do 3º Bimestre:</strong>
            <?php 
            $classe_ano_local = $turma_info['ano'] ?? 0;
            $is_classe_exame_local = ($classe_ano_local == 6 || $classe_ano_local == 9 || $classe_ano_local == 12);
            ...
            if ($is_classe_exame_local): ?>
                <div class="mb-2">
                    <span class="badge bg-warning text-dark">Classe de Exame (<?php echo $classe_ano_local; ?>ª)</span>
                    <strong>NPT não é considerado</strong>
                </div>
                <div>Média = 40% (MAC) + 60% (Exame)</div>
                <?php if ($is_disciplina_lingua_local): ?>
                    <div class="mt-1">📖 <strong>Disciplina de Língua:</strong> Exame = (Oral + Escrito) / 2</div>
                <?php endif; ?>
                <div class="mt-1">⚠️ Se houver <strong>Exame Recurso</strong>, a nota final será a nota do recurso</div>
            <?php else: ?>
                <div>Média = (MAC + NPT) / 2</div>
                <div>Com exame: Média Final = (Média Parcial + Exame) / 2</div>
                <div>⚠️ Se houver <strong>Exame Recurso</strong>, substitui o Exame Normal</div>
            <?php endif; ?>
        </div>
        <?php endif; ?>
                ...
                <table class="pauta-table">
                    <thead>
                        <tr>
                            ...
                            <!-- NPT: Só aparece para classes NÃO exame -->
                    ...
                    <tbody>
                        <?php foreach ($alunos as $index => $aluno): 
                            ...
                            $cor_mac = getCorNota($aluno['mac'], $is_ensino_fundamental);
                            $cor_npt = getCorNota($aluno['npt'], $is_ensino_fundamental);
                        ...
                        <tr>
                            ...
                            <td class="text-center <?php echo $cor_mac; ?>"><?php echo formatarNota($aluno['mac']); ?></td>
                            <!-- NPT: Só aparece para classes NÃO exame -->
                            <?php if ($bimestre != 3 || ($bimestre == 3 && !$is_classe_exame)): ?>
                            <td class="text-center <?php echo $cor_npt; ?>"><?php echo formatarNota($aluno['npt']); ?></td>
                            <?php endif; ?>
                            <!-- Exames para classes de exame -->
                            <?php if ($bimestre == 3 && $is_classe_exame && !$is_disciplina_lingua): ?>
                            <td class="text-center <?php echo $cor_exame_normal; ?>"><?php echo formatarNota($aluno['exame_normal']); ?></td>
                            <td class="text-center <?php echo $cor_exame_recurso; ?>"><?php echo formatarNota($aluno['exame_recurso']); ?></td>
                            <?php elseif ($bimestre == 3 && $is_classe_exame && $is_disciplina_lingua): ?>
                            <td class="text-center <?php echo $cor_exame_oral; ?>"><?php echo formatarNota($aluno['exame_oral']); ?></td>
                            <td class="text-center <?php echo $cor_exame_escrito; ?>"><?php echo formatarNota($aluno['exame_escrito']); ?></td>
                            <td class="text-center <?php echo $cor_exame_recurso; ?>"><?php echo formatarNota($aluno['exame_recurso']); ?></td>
                            <?php elseif ($bimestre == 3): ?>
                            <td class="text-center <?php echo $cor_exame_normal; ?>"><?php echo formatarNota($aluno['exame_normal']); ?></td>
                            <?php endif; ?>
                            <td class="text-center <?php echo $cor_media; ?>"><strong><?php echo number_format($media, 1); ?></strong></td>
