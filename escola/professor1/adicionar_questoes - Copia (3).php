        ...
        if (empty($enunciado)) {
            ...
            try {
                ...
                if ($tipo == 'multipla_escolha' && !empty($alternativas)) {
                    $sql_alt = "INSERT INTO online_provas_alternativas (questao_id, texto, correta, ordem) VALUES (:questao_id, :texto, :correta, :ordem)";
                    ...
                    $sql_vf = "INSERT INTO online_provas_alternativas (questao_id, texto, correta, ordem) VALUES 
                               (:questao_id, 'Verdadeiro', :correta_v, 1),
                               (:questao_id, 'Falso', :correta_f, 2)";
                    $stmt_vf = $conn->prepare($sql_vf);
                    $stmt_vf->execute([
                        ':questao_id' => $questao_nova_id,
                        ':correta_v' => $correta_v,
                        ':correta_f' => $correta_f
...
include '../includes/menu_professor.php';
?>
...
<script>
    ...
    function mudarTipoQuestao() {
        ...
        if (tipo == 'multipla_escolha') {
            ...
            // Não sobrescrever o conteúdo se já existir
            const container = document.getElementById('alternativas_container');
