            ...
            try {
                if ($questao_id > 0) {
                    ...
                    $sql = "INSERT INTO online_provas_questoes (prova_id, enunciado, tipo, pontuacao, ordem) 
                            VALUES (:prova_id, :enunciado, :tipo, :pontuacao, (SELECT COALESCE(MAX(ordem), 0) + 1 FROM online_provas_questoes WHERE prova_id = :prova_id2))";
                    $stmt = $conn->prepare($sql);
                    $stmt->execute([
                        ':prova_id' => $prova_id,
                        ':enunciado' => $enunciado,
                ...
                // Adicionar alternativas (para múltipla escolha e verdadeiro/falso)
