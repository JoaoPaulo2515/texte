...
// ============================================
// ============================================
// FUNÇÃO PARA VISUALIZAR MATERIAL (ATUALIZADA)
// ============================================
// ============================================
...
if (isset($_GET['visualizar']) && isset($_GET['id'])) {
    ...
    if ($material) {
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 text-end">
                            <div class="links-container justify-content-end">
                                ' . (!empty($material['link_pdf']) ? '<a href="biblioteca.php?download_pdf=' . $material['id'] . '" class="btn btn-danger btn-link-material"><i class="fas fa-file-pdf"></i> Baixar PDF</a>' : '') . '
                                ...
                                ' . (!empty($material['link_material']) ? '<a href="' . $material['link_material'] . '" target="_blank" class="btn btn-success btn-link-material"><i class="fas fa-download"></i> Material Extra</a>' : '') . '
                            </div>
                        </div>
                    </div>
                </div>
        ...
        if (!empty($material['link_video'])) {
            ...
            echo '<iframe src="' . $material['link_pdf'] . '" class="pdf-viewer"></iframe>';
...
// ============================================
// FUNÇÃO PARA BAIXAR MATERIAL (ATUALIZADA)
// ============================================
    ...
    if ($material) {
        ...
        if (!empty($arquivo)) {
            ...
            if (file_exists($caminho_completo)) {
                ...
                echo "<h3>Arquivo não encontrado no servidor.</h3>";
                exit;
            }
        }
        
        // Se for conteúdo HTML
        if (!empty($material['conteudo'])) {
            $nome_arquivo = preg_replace('/[^a-zA-Z0-9]/', '_', $material['titulo']) . '.html';
...
if (isset($_GET['download']) && isset($_GET['id'])) {
    ...
    if ($material) {
        ...
        if (!empty($arquivo) && file_exists(__DIR__ . '/../../' . $arquivo)) {
            // Forçar download do arquivo local
            $caminho_completo = __DIR__ . '/../../' . $arquivo;
            header('Content-Description: File Transfer');
            header('Content-Type: application/octet-stream');
            header('Content-Disposition: attachment; filename="' . basename($arquivo) . '"');
            ...
            $nome_arquivo = sanitize_filename($material['titulo']) . '.html';
            header('Content-Type: text/html');
            header('Content-Disposition: attachment; filename="' . $nome_arquivo . '"');
            echo '<!DOCTYPE html>
            <html>
            <head><meta charset="UTF-8"><title>' . htmlspecialchars($material['titulo']) . '</title></head>
            <body>' . $conteudo . '</body>
            </html>';
            exit;
        } else {
...
$funcionario_id = $funcionario_data ? $funcionario_data['id'] : 0;
*/
// ============================================
...
// ============================================
// SOLICITAR EMPRÉSTIMO (CORRIGIDO)
// ============================================
...
<body>
                        ...
                        <div class="material-card position-relative">
                            ...
                            <div class="material-body">
                                ...
                                <div class="d-flex gap-2">
                                    <button class="btn btn-sm btn-outline-primary flex-grow-1" onclick="visualizarMaterial(<?php echo $material['id']; ?>, '<?php echo addslashes($material['arquivo']); ?>')">
    ...
    <script>
        ...
        /*
        function visualizarMaterial(id, arquivo) {
            fetch('biblioteca.php?visualizar=' + id, { method: 'POST' });
            if (arquivo && arquivo != '') {
                window.open(arquivo, '_blank');
            } else {
                alert('Material não disponível para visualização online.');
            }
        }*/
       function visualizarMaterial(id, arquivo) {
    // Abrir em nova aba/janela
    window.open('biblioteca.php?visualizar=' + id, '_blank', 'width=1200,height=800');
}
        
        // Atualizar visualizações
        $('.material-card').on('click', '.btn-outline-primary', function(e) {
            // A visualização já é contada no visualizarMaterial
        });
    </script>
</body>
</html>