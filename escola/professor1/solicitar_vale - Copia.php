...
// ============================================
// PROCESSAR SOLICITAÇÃO COM UPLOAD
// ============================================
$success = '';
$error = '';
        ...
        try {
            ...
            if (isset($_FILES['documento_anexo']) && $_FILES['documento_anexo']['error'] === UPLOAD_ERR_OK) {
                ...
                if (in_array($extensao, $extensoes_permitidas)) {
                    ...
                    $error = "Tipo de arquivo não permitido. Use: PDF, JPG, PNG, DOC";
                    throw new Exception($error);
                }
            }
            ...
            $sql_hist = "INSERT INTO vale_historico (solicitacao_id, acao, observacao) VALUES (:id, 'solicitado', :obs)";
            ...
            $carta_path = gerarCartaModelo($solicitacao_id, $funcionario, $valor_solicitado, $motivo, $descricao, $parcelas);
...
// ============================================
// FUNÇÃO PARA GERAR CARTA MODELO (PDF/HTML)
// ============================================
function gerarCartaModelo($solicitacao_id, $funcionario, $valor, $motivo, $descricao, $parcelas) {
    ...
    <head>
        ...
        <style>
            ...
            .detalhes td { padding: 5px; }
            .detalhes td:first-child { font-weight: bold; width: 40%; }
        </style>
    </head>
    <body>
        <div class="header">
        ...
        <div class="conteudo">
            ...
            <div class="detalhes">
                <table>
                    <tr><td>📅 Data da Solicitação:</td><td>' . $data_atual . '</td></tr>
                    <tr><td>💰 Valor Total:</td><td><strong>KZ ' . $valor_extenso . '</strong></td></tr>
                    <tr><td>📊 Número de Parcelas:</td><td>' . $parcelas_extenso . '</td></tr>
                    <tr><td>💵 Valor por Parcela:</td><td>KZ ' . $valor_parcela_extenso . '</td></tr>
            ...
            <p>Declaro estar ciente de que o valor solicitado será descontado diretamente do meu salário em <strong>' . $parcelas_extenso . '</strong>, conforme planilha de amortização abaixo:</p>
            
            <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
                <thead>
                    <tr style="background: #006B3E; color: white;">
                        <th style="padding: 10px; border: 1px solid #ddd;">Parcela</th>
                        <th style="padding: 10px; border: 1px solid #ddd;">Data Prevista</th>
    ...
    for ($i = 1; $i <= $parcelas; $i++) {
        ...
        $carta .= '
                    <tr>
                        <td style="padding: 8px; border: 1px solid #ddd; text-align: center;">' . $i . 'ª</td>
                        <td style="padding: 8px; border: 1px solid #ddd; text-align: center;">' . $data_parcela . '</td>
                        <td style="padding: 8px; border: 1px solid #ddd; text-align: right;">KZ ' . $valor_parcela_extenso . '</td>
                    </tr>';
    }
...
// ============================================
// GERAR PDF DA CARTA (opcional - usando dompdf)
// ============================================
if (isset($_GET['gerar_pdf']) && isset($_GET['id'])) {
    $id = (int)$_GET['id'];
    $sql = "SELECT carta_path FROM solicitacoes_vale WHERE id = :id AND funcionario_id = :fid";
    $stmt = $conn->prepare($sql);
    $stmt->execute([':id' => $id, ':fid' => $funcionario_id]);
    $carta = $stmt->fetch(PDO::FETCH_ASSOC);
    if ($carta && file_exists(__DIR__ . '/../../' . $carta['carta_path'])) {
        // Aqui você pode implementar a geração de PDF usando dompdf ou similar
        // Por enquanto, redireciona para visualização HTML
        header('Location: ' . $carta['carta_path']);
        exit;
    }
}
...
$sql_solicitacoes = "
    ...
    LEFT JOIN dividas_a_receber d ON d.id = s.divida_id
...
<body>
                            ...
                            <div class="solicitacao-card <?php echo $s['status']; ?>" data-id="<?php echo $s['id']; ?>">
                                <div class="d-flex justify-content-between align-items-center">
                                    <div>
                                        ...
                                        <br>
                                        <small class="text-muted"><?php echo formatarData($s['data_solicitacao']); ?></small>
                                    </div>
                                    <div>
    ...
    <script>
        ...
        // Calcular e mostrar valor da parcela em tempo real
        document.getElementById('valor_solicitado').addEventListener('input', calcularParcela);
        document.getElementById('parcelas').addEventListener('change', calcularParcela);
        
        function calcularParcela() {
            let valor = parseFloat(document.getElementById('valor_solicitado').value);
            let parcelas = parseInt(document.getElementById('parcelas').value);
            if (valor && parcelas && !isNaN(valor) && !isNaN(parcelas) && parcelas > 0) {
                let parcela = valor / parcelas;
                // Mostrar informação adicional se desejar
            }
        }
    </script>
</body>
