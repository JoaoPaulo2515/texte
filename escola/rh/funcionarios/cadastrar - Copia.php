...
// Função para validar BI de Angola (aceita maiúsculas e minúsculas)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cadastrar_funcionario'])) {
    ...
    if (!$error) {
        try {
            ...
            // INSERIR FUNCIONÁRIO - CORRIGIDO com todos os campos
            $stmt = $conn->prepare("
            ...
            // ============================================
            // PROCESSAR DOCUMENTOS (ESTILO PROFESSORES)
            // ============================================
            ...
            // Array de documentos (igual ao cadastro de professores)
            $documentos = [
...
<head>
    ...
    <style>
        ...
        .scanner-video { width: 100%; border-radius: 10px; background: #000; }
        .scanner-canvas { display: none; }
        .formato-badge { position: absolute; top: 10px; right: 10px; background: rgba(0,0,0,0.7); color: white; padding: 2px 8px; border-radius: 5px; font-size: 11px; }
    ...
    <script>
        ...
        function toggleSubmenu(event) {
            ...
            if (parent) {
                ...
                parent.querySelector('.nav-submenu').classList.toggle('show');
            }
        }
        ...
        function previewDocumento(input, previewId) {
            ...
            if (input.files && input.files[0]) {
                ...
                if (file.type.startsWith('image/')) {
                    ...
                    reader.onload = function(e) { preview.html(`<img src="${e.target.result}" class="document-preview">`); };
        ...
        function capturarDocumento() {
            ...
            $.ajax({
                url: 'cadastrar.php', method: 'POST', data: { acao: 'upload_documento_scanner', tipo_documento: currentTipo, imagem: imgData },
                success: function(res) {
                    ...
                    if (r.success) {
                        ...
                        $('#' + currentPreviewId).html(`<div style="position:relative"><img src="${r.imagem}" style="max-width:60px"><span class="badge bg-info formato-badge">${r.formato}</span></div>`);
        ...
        $('#salvarProvinciaBtn').click(function() {
            const nome = $('#novaProvincia').val();
            if (nome) $.ajax({ url: 'cadastrar.php', method: 'POST', data: { acao: 'add_provincia', nova_provincia: nome },
                success: function(d) { const r = JSON.parse(d); if(r.success) { $('#provincia_id').append(`<option value="${r.id}">${r.nome}</option>`).val(r.id).trigger('change'); $('#modalNovaProvincia').modal('hide'); $('#novaProvincia').val(''); } }
            });
        });
        
        $('#salvarMunicipioBtn').click(function() {
            const nome = $('#novoMunicipio').val(), provId = $('#provincia_id').val();
            if (nome && provId) $.ajax({ url: 'cadastrar.php', method: 'POST', data: { acao: 'add_municipio', novo_municipio: nome, provincia_id: provId },
                success: function(d) { const r = JSON.parse(d); if(r.success) { $('#municipio_id').append(`<option value="${r.id}">${r.nome}</option>`).val(r.id).trigger('change'); $('#modalNovoMunicipio').modal('hide'); $('#novoMunicipio').val(''); } }
            });
        });
        
        $('#salvarComunaBtn').click(function() {
            const nome = $('#novaComuna').val(), munId = $('#municipio_id').val();
            if (nome && munId) $.ajax({ url: 'cadastrar.php', method: 'POST', data: { acao: 'add_comuna', nova_comuna: nome, municipio_id: munId },
                success: function(d) { const r = JSON.parse(d); if(r.success) { $('#comuna_id').append(`<option value="${r.id}">${r.nome}</option>`).val(r.id); $('#modalNovaComuna').modal('hide'); $('#novaComuna').val(''); } }
            });
        });
        
        $('#modalNovoMunicipio').on('show.bs.modal', function() { $('#municipioProvincia').val($('#provincia_id option:selected').text() || 'Selecione uma província'); });
        $('#modalNovaComuna').on('show.bs.modal', function() { $('#comunaMunicipio').val($('#municipio_id option:selected').text() || 'Selecione um município'); });
        
        const video = document.getElementById('video'), canvas = document.getElementById('canvas');
        let webcamStream = null;
        navigator.mediaDevices.getUserMedia({ video: true }).then(s => { webcamStream = s; video.srcObject = s; }).catch(e => console.log(e));
        document.getElementById('recarregarCamBtn')?.addEventListener('click', () => { if(webcamStream) webcamStream.getTracks().forEach(t => t.stop()); navigator.mediaDevices.getUserMedia({ video: true }).then(s => { webcamStream = s; video.srcObject = s; }); });
        document.getElementById('capturarBtn')?.addEventListener('click', function() {
            canvas.width = video.videoWidth; canvas.height = video.videoHeight;
            canvas.getContext('2d').drawImage(video, 0, 0);
            const data = canvas.toDataURL('image/png');
            $('#fotoPreview').attr('src', data);
            fetch(data).then(r => r.blob()).then(b => { const f = new File([b], 'foto.png', { type: 'image/png' }); const dt = new DataTransfer(); dt.items.add(f); document.getElementById('fotoInput').files = dt.files; });
        ...
        document.getElementById('formFuncionario')?.addEventListener('submit', function(e) {
            ...
            if (bi && !/^[0-9]{9}[A-Z]{2}[0-9]{3}$/.test(bi.toUpperCase())) { e.preventDefault(); alert('BI inválido!'); return false; }
            const tel = document.querySelector('input[name="telefone"]').value;
            if (!/^9[1-9][0-9]{7}$/.test(tel)) { e.preventDefault(); alert('Telefone inválido!'); return false; }
            return true;
        });
    </script>
</body>
