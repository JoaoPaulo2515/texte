...
// Buscar responsável para assinar (Diretor > RH > Admin)
$responsavel = null;

// 1. Tentar buscar Diretor
$stmt = $conn->prepare("
    SELECT u.nome, f.cargo
    FROM usuarios u 
    INNER JOIN funcionarios f ON f.usuario_id = u.id
    WHERE u.escola_id = ? 
    AND (f.cargo LIKE '%Diretor%' OR f.cargo LIKE '%Director%')
    AND u.status = 'ativo'
    LIMIT 1
");
$stmt->execute([$escola_id]);
...
// 2. Se não encontrar Diretor, tentar RH
if (!$responsavel) {
    $stmt = $conn->prepare("
        SELECT u.nome, f.cargo
        FROM usuarios u 
        INNER JOIN funcionarios f ON f.usuario_id = u.id
        WHERE u.escola_id = ? 
        AND (f.cargo LIKE '%RH%' OR f.cargo LIKE '%Recursos Humanos%' OR f.cargo LIKE '%Recurso Humanos%')
        AND u.status = 'ativo'
        LIMIT 1
    ");
    $stmt->execute([$escola_id]);
...
// 3. Se não encontrar RH, tentar Admin
if (!$responsavel) {
    $stmt = $conn->prepare("
        SELECT u.nome, f.cargo
        FROM usuarios u 
        LEFT JOIN funcionarios f ON f.usuario_id = u.id
        WHERE u.escola_id = ? AND u.tipo = 'admin' AND u.status = 'ativo'
        LIMIT 1
    ");
    $stmt->execute([$escola_id]);
    $responsavel = $stmt->fetch(PDO::FETCH_ASSOC);
...
// 4. Se ainda não encontrar, usar valores padrão com xxxxxx
if (!$responsavel) {
    $responsavel = [
...
// Gerar conteúdo HTML do PDF (sem a seção Informações do Funcionário)
$html = '
<!DOCTYPE html>
<html lang="pt-AO">
<head>
...
<div class="conteudo">
    <p class="texto-declaracao">
        Eu, <strong>' . htmlspecialchars($responsavel['nome']) . '</strong>, na qualidade de <strong>' . htmlspecialchars($responsavel['cargo']) . '</strong> da instituição 
...
<div class="assinatura">
    ...
    <div class="assinatura-nome">' . htmlspecialchars($responsavel['nome']) . '</div>
    <div class="assinatura-cargo">' . htmlspecialchars($responsavel['cargo']) . '</div>
...
// CORREÇÃO DO CAMINHO - Usando DIRECTORY_SEPARATOR para compatibilidade
$base_upload_dir = dirname(__DIR__, 4) . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR;
$declaracoes_dir = $base_upload_dir . 'declaracoes' . DIRECTORY_SEPARATOR;

// Criar diretórios recursivamente
criarDiretorio($base_upload_dir);
criarDiretorio($declaracoes_dir);

$pdf_file = $declaracoes_dir . $nome_arquivo;
$pdf_url_relativo = '../../../uploads/declaracoes/' . $nome_arquivo;
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <ul class="nav-menu">
            <li class="nav-item"><a href="../index.php" class="nav-link"><i class="fas fa-chart-line"></i> Dashboard RH</a></li>
            <li class="nav-item has-submenu open" id="menuFuncionarios">
                <a href="#" class="nav-link active" onclick="toggleSubmenu(event)"><i class="fas fa-user-tie"></i> Funcionários</a>
                <ul class="nav-submenu show" id="submenuFuncionarios">
                    <li class="nav-item"><a href="listar.php" class="nav-link"><i class="fas fa-list"></i> Listar</a></li>
                    <li class="nav-item"><a href="cadastrar.php" class="nav-link"><i class="fas fa-user-plus"></i> Cadastrar</a></li>
                    <li class="nav-item"><a href="visualizar.php?id=<?php echo $id; ?>" class="nav-link"><i class="fas fa-eye"></i> Visualizar</a></li>
                </ul>
            </li>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="alert alert-secondary">
                            ...
                            <strong><i class="fas fa-id-card"></i> Nº Processo:</strong> <?php echo htmlspecialchars($funcionario['numero_processo']); ?><br>
                            <strong><i class="fas fa-briefcase"></i> Cargo:</strong> <?php echo htmlspecialchars($funcionario['cargo']); ?>
                        </div>
                        
                        <div class="alert alert-warning">
                            <i class="fas fa-user-tie"></i> 
                            <strong>Responsável pela Assinatura:</strong><br>
                            <?php echo htmlspecialchars($responsavel['nome']); ?><br>
                            <small><?php echo htmlspecialchars($responsavel['cargo']); ?></small>
                        </div>
                        ...
                        <div class="d-grid gap-2">
                            ...
                            <a href="<?php echo $pdf_url_relativo; ?>" target="_blank" class="btn btn-primary btn-lg">
            ...
            <div class="col-md-8">
                <div class="card">
                    ...
                    <div class="card-body text-center">
                        <?php if ($salvo_com_sucesso && file_exists($pdf_file)): ?>
                            <iframe src="<?php echo $pdf_url_relativo; ?>" class="preview-pdf" frameborder="0"></iframe>
                            <div class="mt-3">
                        ...
                        <?php else: ?>
                            <div class="alert alert-warning">
                                ...
                                <strong>Clique em "Baixar PDF" para gerar o documento.</strong>
