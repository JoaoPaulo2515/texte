<?php
// escola/secretaria/matricula.php - Cadastro de Aluno (Versão Secretaria) com Scanner de Documentos
...
<head>
    ...
    <!-- Bibliotecas para Scanner de Documentos -->
    ...
    <style>
        ...
        /* Scanner de Documentos - Estilo iPhone */
        ...
        @keyframes pulse {
            0% { border-color: rgba(0, 255, 100, 0.8); box-shadow: 0 0 0 9999px rgba(0, 0, 0, 0.5); }
            50% { border-color: rgba(0, 255, 100, 1); box-shadow: 0 0 0 9999px rgba(0, 0, 0, 0.3); }
            100% { border-color: rgba(0, 255, 100, 0.8); box-shadow: 0 0 0 9999px rgba(0, 0, 0, 0.5); }
        }
        .scanner-controls {
            position: absolute;
            bottom: 20px;
...
<body>
    ...
    <!-- Modal Scanner de Documentos (Estilo iPhone) -->
            ...
            <div class="modal-content">
                ...
                <div class="modal-body p-0">
                    ...
                    <div id="scannerResult" class="scanner-result text-center" style="display: none;">
                        <p class="mb-2"><i class="fas fa-check-circle text-success"></i> Documento digitalizado!</p>
                        <img id="scannedImage" src="" style="max-width: 100%; max-height: 200px; border-radius: 10px;">
                        <div class="mt-2">
                            <button type="button" id="aceitarScannerBtn" class="btn btn-sm btn-success">Aceitar</button>
                            <button type="button" id="novamenteScannerBtn" class="btn btn-sm btn-secondary">Digitalizar Novamente</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        function capturarImagemScanner() {
            ...
            // Aplicar correções de perspectiva e melhorias (simulação de scanner)
            const imagemData = canvas.toDataURL('image/jpeg', 0.9);
            
            // Mostrar resultado
            scannedImageData = imagemData;
            document.getElementById('scannedImage').src = imagemData;
            ...
            if (scannedImageData && currentTargetInput) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (result.success) {
                            // Atualizar o campo hidden com o nome do arquivo
                            $('#' + currentTargetInput).val(result.arquivo);
                            // Mostrar preview
                            $('#' + currentTargetPreview).html('<img src="../../uploads/alunos/documentos/' + result.arquivo + '" class="document-preview" onclick="window.open(this.src)">');
                            // Fechar modal
        ...
        // Eventos do Scanner
        $('#recarregarCamScannerBtn').click(function() {
            iniciarScanner();
        });
        
        $('#capturarScannerBtn').click(function() {
            capturarImagemScanner();
        });
        
        $('#aceitarScannerBtn').click(function() {
            aceitarDocumento();
        });
        
        $('#novamenteScannerBtn').click(function() {
            document.querySelector('.scanner-container').style.display = 'block';
            document.getElementById('scannerResult').style.display = 'none';
            iniciarScanner();
        });
