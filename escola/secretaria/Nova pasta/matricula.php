<?php
// escola/secretaria/matricula.php - Cadastro de Aluno (Versão Secretaria)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cadastrar_aluno'])) {
    ...
    // Credenciais de acesso: usuário = número de matrícula, senha = BI
    ...
    try {
        ...
        $stmt = $conn->prepare("
            INSERT INTO estudantes (
                ...
                ano_letivo, ano_escolar, curso, nivel, turno, sala,
...
<head>
    ...
    <style>
        ...
        .modal-document-capture { max-width: 600px; }
        .capture-canvas { width: 100%; border-radius: 10px; background: #000; }
        .btn-capture-doc { background: #006B3E; color: white; }
    </style>
</head>
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <ul class="nav-menu">
            ...
            <li class="nav-item has-submenu open" id="menuSecretaria">
                <a href="#" class="nav-link active" onclick="toggleSubmenu(event)"><i class="fas fa-building"></i> Secretaria</a>
                <ul class="nav-submenu show" id="submenuSecretaria">
                    ...
                    <li class="nav-item"><a href="matricula.php" class="nav-link active"><i class="fas fa-user-plus"></i> Matrícula</a></li>
                </ul>
            </li>
                                ...
                                <div class="col-md-6">
                                    <div class="card mb-3">
                                        <div class="card-body text-center">
                                            ...
                                            <button type="button" class="btn btn-sm btn-capture-doc mt-2" onclick="abrirModalCaptura('bi_documento', 'bi_preview')">
                                ...
                                <div class="col-md-6">
                                    <div class="card mb-3">
                                        <div class="card-body text-center">
                                            ...
                                            <button type="button" class="btn btn-sm btn-capture-doc mt-2" onclick="abrirModalCaptura('certificado_documento', 'certificado_preview')">
                                                <i class="fas fa-camera"></i> Capturar Documento
                                            </button>
                                            <small class="text-muted d-block">PDF, JPG, PNG (Max: 2MB)</small>
                                        </div>
                                ...
                                <div class="col-md-6">
                                    <div class="card mb-3">
                                        <div class="card-body text-center">
                                            ...
                                            <button type="button" class="btn btn-sm btn-capture-doc mt-2" onclick="abrirModalCaptura('atestado_documento', 'atestado_preview')">
                                                <i class="fas fa-camera"></i> Capturar Documento
                                            </button>
                                            <small class="text-muted d-block">PDF, JPG, PNG (Max: 2MB)</small>
                                        </div>
                                ...
                                <div class="col-md-6">
                                    <div class="card mb-3">
                                        <div class="card-body text-center">
                                            ...
                                            <button type="button" class="btn btn-sm btn-capture-doc mt-2" onclick="abrirModalCaptura('declaracao_documento', 'declaracao_preview')">
                                                <i class="fas fa-camera"></i> Capturar Documento
                                            </button>
                                            <small class="text-muted d-block">PDF, JPG, PNG (Max: 2MB)</small>
                                        </div>
    ...
    <!-- Modal Captura de Documento -->
    <div class="modal fade" id="modalCapturaDocumento" tabindex="-1">
        <div class="modal-dialog modal-document-capture">
            <div class="modal-content">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title"><i class="fas fa-camera"></i> Capturar Documento</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body text-center">
                    <video id="videoDoc" width="100%" autoplay style="border-radius: 10px; background: #000;"></video>
                    <canvas id="canvasDoc" style="display:none;"></canvas>
                    <div class="mt-3">
                        <button type="button" id="capturarDocBtn" class="btn btn-primary"><i class="fas fa-camera"></i> Capturar</button>
                        <button type="button" id="recarregarDocCamBtn" class="btn btn-secondary"><i class="fas fa-sync-alt"></i> Recarregar</button>
                    </div>
                </div>
    ...
    <script>
        ...
        // ============================================
        // CAPTURA DE DOCUMENTO COM WEBCAM
        // ============================================
        let docStream = null;
        let targetInput = null;
        let targetPreview = null;
        
        function abrirModalCaptura(inputName, previewId) {
            targetInput = inputName;
            targetPreview = previewId;
            iniciarWebcamDocumento();
            $('#modalCapturaDocumento').modal('show');
        }
        
        function iniciarWebcamDocumento() {
            if (docStream) {
            ...
            navigator.mediaDevices.getUserMedia({ video: true })
                .then(function(mediaStream) {
                    ...
                    const videoDoc = document.getElementById('videoDoc');
                    videoDoc.srcObject = mediaStream;
                })
                .catch(function(err) {
        ...
        $('#recarregarDocCamBtn').click(function() {
            iniciarWebcamDocumento();
        });
        
        $('#capturarDocBtn').click(function() {
            const videoDoc = document.getElementById('videoDoc');
            const canvasDoc = document.getElementById('canvasDoc');
            canvasDoc.width = videoDoc.videoWidth;
            canvasDoc.height = videoDoc.videoHeight;
            canvasDoc.getContext('2d').drawImage(videoDoc, 0, 0, canvasDoc.width, canvasDoc.height);
            
            const fotoData = canvasDoc.toDataURL('image/png');
            
            // Converter para Blob e criar arquivo
            fetch(fotoData)
                .then(res => res.blob())
                .then(blob => {
                    const file = new File([blob], 'documento_' + Date.now() + '.png', { type: 'image/png' });
                    const dataTransfer = new DataTransfer();
                    dataTransfer.items.add(file);
                    
                    // Encontrar o input file e atualizar
                    const inputFile = document.querySelector(`input[name="${targetInput}"]`);
                    if (inputFile) {
                        inputFile.files = dataTransfer.files;
                        // Trigger change event para preview
                        const event = new Event('change', { bubbles: true });
                        inputFile.dispatchEvent(event);
                        previewDocumento(inputFile, targetPreview);
                    }
                    
                    $('#modalCapturaDocumento').modal('hide');
        ...
        $('#modalCapturaDocumento').on('hidden.bs.modal', function() {
            if (docStream) {
                docStream.getTracks().forEach(track => track.stop());
            }
        });
