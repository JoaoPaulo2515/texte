        ...
        if (empty($cart)) {
            ...
            try {
                ...
                foreach ($cart as $item) {
                    // ANTES DE INSERIR, VERIFICAR NOVAMENTE
                    ...
                    // Registrar pagamento
                  // Registrar pagamento - COM campos extras (depois de adicionar as colunas)
$sql = "INSERT INTO pagamentos (escola_id, assinatura_id, tipo_pagamento, valor, metodo_pagamento, referente, observacoes, data_pagamento, usuario_id, status, numero_fatura, numero_referencia, comprovativo_path, quem_recebeu, quem_pagou, created_at) 
        VALUES (:escola_id, :assinatura_id, :tipo_pagamento, :valor, :forma_pagamento, :referente, :observacoes, CURDATE(), :usuario_id, 'confirmado', :numero_fatura, :numero_referencia, :comprovativo_path, :quem_recebeu, :quem_pagou, NOW())";
...
$stmt->execute([
    ...
    ':comprovativo_path' => $comprovativo_path,
    ...
    ':quem_pagou' => $item['quem_pagou']
                    ...
                    if ($item['tipo_pagamento_nome'] == 'mensalidade' && $item['mes_referencia'] > 0) {
                        $sql_update = "UPDATE mensalidades SET valor_pago = valor_pago + :valor, status = CASE WHEN valor_pago + :valor >= valor_total THEN 'pago' ELSE 'parcial' END, data_pagamento = CURDATE() 
                                       WHERE escola_id = :escola_id AND aluno_id = :aluno_id AND mes_referencia = :mes AND ano_referencia = :ano";
                    ...
                    $stmt_caixa->execute([
                        ...
                        ':descricao' => "Pagamento de " . ucfirst($item['tipo_pagamento_nome']) . " - " . $item['aluno_nome'],
                        ':valor' => $item['valor'],
                        ':forma_pagamento' => $forma_pagamento,
...
$sql_debitos = "SELECT m.*, e.nome as aluno_nome, e.matricula FROM mensalidades m JOIN estudantes e ON e.id = m.aluno_id WHERE m.escola_id = :escola_id AND m.status IN ('pendente', 'parcial') ORDER BY e.nome ASC, m.ano_referencia DESC, m.mes_referencia ASC LIMIT 10";
...
$sql_ultimos = "SELECT p.*, e.nome as aluno_nome, e.matricula FROM pagamentos p JOIN estudantes e ON e.id = p.assinatura_id WHERE p.escola_id = :escola_id ORDER BY p.id DESC LIMIT :offset, :limit";
$stmt_ultimos = $conn->prepare($sql_ultimos);
$stmt_ultimos->bindParam(':offset', $offset, PDO::PARAM_INT);
$stmt_ultimos->bindParam(':limit', $registros_por_pagina, PDO::PARAM_INT);
$stmt_ultimos->execute();
$ultimos_pagamentos = $stmt_ultimos->fetchAll(PDO::FETCH_ASSOC);

// Contar total para paginação
$sql_total = "SELECT COUNT(*) as total FROM pagamentos WHERE escola_id = :escola_id";
$stmt_total = $conn->prepare($sql_total);
$stmt_total->execute([':escola_id' => $escola_id]);
...
<body>
    ...
    <div class="main-content-tesouraria">
        ...
        <!-- Dívidas em Aberto -->
        <div class="card mb-4">
            <div class="card-header"><h5 class="mb-0"><i class="fas fa-exclamation-triangle"></i> Dívidas em Aberto</h5></div>
            <div class="card-body">
                <?php if (empty($debitos)): ?>
                    <div class="alert alert-success text-center">Nenhuma dívida em aberto no momento!</div>
                <?php else: ?>
                    <div class="table-responsive">
                        <table class="table table-hover">
                            <thead class="table-light"><tr><th>Aluno</th><th>Mês/Ano</th><th>Valor Total</th><th>Valor Pago</th><th>Saldo Devedor</th><th>Status</th><th>Ação</th></tr></thead>
                            <tbody>
                                ...
                                <tr class="debito-row">
                                    ...
                                    <td class="valor-debito"><?php echo formatarMoeda(($debito['valor_total'] - ($debito['valor_pago'] ?? 0))); ?><tr>
        ...
        <div class="card">
            <div class="card-header"><h5 class="mb-0"><i class="fas fa-history"></i> Últimos Pagamentos</h5></div>
            <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover table-pagamentos">
                            <thead class="table-light">
                                <tr><th>Data</th><th>Aluno</th><th>Tipo</th><th>Valor</th><th>Forma</th><th>Quem Pagou</th><th>Referência</th></tr>
                            </thead>
                            <tbody>
                        ...
                        <div class="row">
                            <div class="col-md-12 mb-3">
                                ...
                                <select name="tipo_pagamento_id" id="tipo_pagamento_select" class="form-select" required onchange="toggleMensalidadeSelect()">
                                    ...
                                    <option value="<?php echo $tipo['id']; ?>" data-nome="<?php echo $tipo['nome']; ?>" data-icone="<?php echo $tipo['icone']; ?>">
                        ...
                        <div class="row" id="div_mensalidade" style="display:none;">
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="mes_referencia" class="form-select">
                                    <?php for($i=1;$i<=12;$i++): ?>
                                    <option value="<?php echo $i; ?>"><?php echo getMesNome($i); ?></option>
                                    <?php endfor; ?>
                                </select>
                            </div>
                            <div class="col-md-6 mb-3">
                    ...
                    <div class="modal-body">
                        ...
                        <?php else: ?>
                            ...
                            <div class="mt-3">
                                ...
                                <input type="text" name="quem_recebeu" class="form-control" value="<?php echo $usuario_nome; ?>" required>
                            </div>
    ...
    <script>
        ...
        // Toggle campos de mensalidade baseado no tipo de pagamento selecionado
        ...
        // Toggle campos de comprovativo baseado na forma de pagamento
