...
$disciplina_id_pre = $_GET['disciplina_id'] ?? 0;
...
// Se veio com disciplina_id via GET, tentar associar automaticamente (associação rápida)
if ($disciplina_id_pre > 0) {
    // Verificar se a disciplina existe e está disponível
    $stmt = $conn->prepare("
        SELECT id FROM disciplinas 
        WHERE id = :id AND escola_id = :escola_id AND status = 'ativa'
    ");
    $stmt->execute([':id' => $disciplina_id_pre, ':escola_id' => $escola_id]);
    $disciplina_check = $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    if ($disciplina_check) {
        ...
        ");
        $stmt->execute([
            ':disciplina_id' => $disciplina_id_pre,
            ':turma_id' => $turma_id,
            ':ano' => $ano_letivo
        ]);
        
        if (!$stmt->fetch()) {
            // Associar automaticamente (sem professor definido - será necessário editar depois)
            try {
                $stmt = $conn->prepare("
                    INSERT INTO alocacoes (professor_id, disciplina_id, turma_id, ano_letivo, created_at)
                    VALUES (NULL, :disciplina_id, :turma_id, :ano, NOW())
                ");
                $stmt->execute([
                    ':disciplina_id' => $disciplina_id_pre,
                    ':turma_id' => $turma_id,
                ...
                $_SESSION['success_message'] = "Disciplina associada com sucesso!";
                header("Location: associar_disciplina.php?turma_id={$turma_id}");
                exit;
            } catch (Exception $e) {
                $error_auto = $e->getMessage();
            }
        } else {
            $error_auto = "Disciplina já está associada a esta turma.";
        }
    }
}
...
// Buscar disciplinas já associadas
$stmt = $conn->prepare("
    ...
    LEFT JOIN professores p_titular ON p_titular.id = a.professor_id
    LEFT JOIN usuarios u_titular ON u_titular.id = p_titular.usuario_id
    ...
    WHERE a.turma_id = :turma_id AND a.ano_letivo = :ano
    GROUP BY d.id
    ORDER BY d.nome
");
$stmt->execute([
...
$disciplinas_associadas = $stmt->fetchAll(PDO::FETCH_ASSOC);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$disciplina_id || !$professor_titular_id) {
        ...
        try {
            ...
            $aloc_existente = $stmt->fetch();
            
            if ($aloc_existente) {
                // Atualizar professor titular
                $stmt = $conn->prepare("
                ...
                $stmt->execute([
                    ...
                    ':id' => $aloc_existente['id']
                ]);
                $aloc_id = $aloc_existente['id'];
            } else {
                ...
                $aloc_id = $conn->lastInsertId();
            }
            
            // Remover professor auxiliar antigo se existir
            $stmt = $conn->prepare("
                DELETE FROM alocacoes 
                WHERE disciplina_id = :disciplina_id AND turma_id = :turma_id AND ano_letivo = :ano AND id != :aloc_id
            ");
            $stmt->execute([
                ':disciplina_id' => $disciplina_id,
                ':turma_id' => $turma_id,
            ...
            $stmt = $conn->prepare("
                ...
                VALUES (:usuario_id, 'associar_disciplina_turma', 'alocacoes', :registro_id, :ip, NOW())
            ");
            $stmt->execute([
    ...
    <div class="main-content">
        ...
        <div class="row">
            <div class="col-md-7">
                <div class="card">
                    <div class="card-header">
                        <h3 class="mb-0">Nova Associação</h3>
                    </div>
                    <div class="card-body">
                        <form method="POST">
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="mb-3">
                                        ...
                                        <select name="disciplina_id" class="form-control" required>
                                            ...
                                            <option value="<?php echo $disc['id']; ?>" <?php echo $disciplina_id_pre == $disc['id'] ? 'selected' : ''; ?>>
                                        ...
                                        <small class="text-danger">Todas as disciplinas já estão associadas a esta turma.</small>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="text-center mt-4">
                                <button type="submit" class="btn btn-primary btn-lg px-5" <?php echo empty($disciplinas_disponiveis) ? 'disabled' : ''; ?>>
            ...
            <div class="col-md-5">
                <div class="card">
                    <div class="card-header">
                        <h3 class="mb-0"><i class="fas fa-list"></i> Disciplinas Associadas</h3>
                    </div>
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="table table-hover mb-0">
                                <thead>
                                    <tr><th>Disciplina</th><th>Professor Titular</th><th>Professor Auxiliar</th><th>Ações</th></tr>
                                </thead>
                                <tbody>
                                    <?php foreach ($disciplinas_associadas as $assoc): ?>
                                    <tr>
                                        <td><strong><?php echo htmlspecialchars($assoc['nome']); ?></strong></td>
                                        <td><?php echo htmlspecialchars($assoc['professor_titular'] ?? '-'); ?></td>
                                        <td><?php echo htmlspecialchars($assoc['professor_auxiliar'] ?? '-'); ?></td>
                                        <td>
                                            <a href="remover_disciplina.php?turma_id=<?php echo $turma_id; ?>&disciplina_id=<?php echo $assoc['id']; ?>" 
                                    ...
                                    <?php endforeach; ?>
                                    <?php if (empty($disciplinas_associadas)): ?>
                                    <tr>
                                        <td colspan="4" class="text-center">Nenhuma disciplina associada</td>
                                    </tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
