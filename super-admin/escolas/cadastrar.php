<?php
// super-admin/escolas/cadastrar.php - Cadastro de Escola (Wizard com Validações em Tempo Real)
...
if (isset($_GET['ajax'])) {
    ...
    if ($ajax == 'verificar_subdominio') {
        ...
        $response = ['valid' => false, 'message' => '', 'sugestoes' => []];
        ...
        if (empty($subdominio)) {
        ...
        } elseif (strlen($subdominio) > 50) {
            $response['message'] = 'Máximo 50 caracteres';
        } else {
            ...
            if ($stmt->fetch()) {
                ...
                $response['sugestoes'] = [$subdominio . rand(1, 99), $subdominio . date('y'), $subdominio . '_escola'];
            } else {
                $response['valid'] = true;
    ...
    if ($ajax == 'verificar_dominio') {
        ...
        if (empty($dominio)) {
            ...
            $stmt = $conn->prepare("SELECT id FROM escolas WHERE dominio_personalizado = :dominio");
    ...
    if ($ajax == 'verificar_nuit') {
        $nuit = $_GET['nuit'] ?? '';
        $response = ['valid' => false, 'message' => '', 'formatado' => ''];
        ...
        if (empty($nuit)) {
            ...
            if (strlen($nuit_limpo) != 14) {
                ...
                // Formatar NUIT para exibição
                $response['formatado'] = substr($nuit_limpo, 0, 3) . '.' . substr($nuit_limpo, 3, 3) . '.' . substr($nuit_limpo, 6, 3) . '.' . substr($nuit_limpo, 9, 5);
                
                $stmt = $conn->prepare("SELECT id, nome FROM escolas WHERE nuit = :nuit");
                $stmt->execute([':nuit' => $nuit_limpo]);
                $existente = $stmt->fetch(PDO::FETCH_ASSOC);
                
                if ($existente) {
                    $response['message'] = 'NUIT já cadastrado para: ' . $existente['nome'];
                } else {
                    $response['valid'] = true;
        ...
        try {
            ...
            echo json_encode([
                ...
                'redirect' => 'index.php'
                        ...
                        // Após salvar a escola com sucesso
            if ($success) {
                // Executar script PowerShell para atualizar hosts
                if (PHP_OS_FAMILY === 'Windows') {
                    $psScript = __DIR__ . '/../../auto_hosts.ps1';
...
<head>
    ...
    <style>
        ...
        .valid-feedback-custom { font-size: 12px; margin-top: 5px; }
        .is-valid-custom { border-color: #28a745; background-image: url('data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="%2328a745"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17z"/></svg>'); background-repeat: no-repeat; background-position: right 10px center; background-size: 20px; }
        .is-invalid-custom { border-color: #dc3545; background-image: url('data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="%23dc3545"><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12 19 6.41z"/></svg>'); background-repeat: no-repeat; background-position: right 10px center; background-size: 20px; }
    </style>
</head>
<body>
    <div class="container">
                        ...
                        <div class="row">
                            <div class="col-md-8">
                                <div class="mb-3">
                                    ...
                                    <input type="text" id="nome_escola" class="form-control" placeholder="Ex: Escola Primária do Futuro" autocomplete="off">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                <div class="mb-3">
                                    ...
                                    <input type="text" id="telefone" class="form-control" placeholder="222 123 456">
                            ...
                            <div class="col-md-3">
                                <div class="mb-3">
                                    <label>Celular</label>
                                    <input type="text" id="celular" class="form-control" placeholder="923 456 789">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <div class="input-group">
                                        <input type="text" id="nuit" class="form-control" placeholder="123.456.789.01234">
                                    </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" id="director_contato" class="form-control" placeholder="923 456 789">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" id="director_pedagogico_contato" class="form-control" placeholder="923 456 789">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" id="secretario_contato" class="form-control" placeholder="923 456 789">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <input type="text" id="responsavel_telefone" class="form-control" placeholder="923 456 789">
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        // Verificar subdomínio em tempo real
        function verificarSubdominioRealtime() {
            ...
            timeoutSubdominio = setTimeout(function() {
                ...
                if (subdominio.length === 0) {
                    ...
                    $('#subdominio').removeClass('is-valid-custom is-invalid-custom');
                    return;
                }
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.valid) {
                            ...
                            $('#subdominio').addClass('is-valid-custom');
                        } else {
                            $('#statusSubdominio').html('<span class="status-badge status-error"><i class="fas fa-times-circle"></i> ' + data.message + '</span>');
                            $('#subdominio').addClass('is-invalid-custom');
                            
                            if (data.sugestoes && data.sugestoes.length > 0) {
                                let suggHtml = '<div class="small text-muted mt-1">Sugestões: ';
                                data.sugestoes.forEach(function(s) {
                                    suggHtml += '<a href="#" onclick="$(\'#subdominio\').val(\'' + s + '\'); verificarSubdominioRealtime(); return false;" class="text-primary">' + s + '</a> ';
                                });
                                suggHtml += '</div>';
                                $('#statusSubdominio').append(suggHtml);
                            }
                        }
                    }
                });
        ...
        // Verificar domínio personalizado em tempo real
        function verificarDominioRealtime() {
            ...
            timeoutDominio = setTimeout(function() {
                ...
                if (dominio.length === 0) {
                    $('#statusDominio').html('');
                    $('#dominio_personalizado').removeClass('is-valid-custom is-invalid-custom');
                    return;
                }
                ...
                $.ajax({
                    url: 'cadastrar.php?ajax=verificar_dominio&dominio=' + encodeURIComponent(dominio),
                    method: 'GET',
                    success: function(data) {
                        if (data.valid) {
                            $('#statusDominio').html('<span class="status-badge status-ok"><i class="fas fa-check-circle"></i> ' + data.message + '</span>');
                            $('#dominio_personalizado').addClass('is-valid-custom').removeClass('is-invalid-custom');
                        } else if (data.message) {
                            $('#statusDominio').html('<span class="status-badge status-error"><i class="fas fa-times-circle"></i> ' + data.message + '</span>');
                            $('#dominio_personalizado').addClass('is-invalid-custom').removeClass('is-valid-custom');
                        }
                    }
                });
            }, 500);
        ...
        function verificarNuitRealtime() {
            ...
            timeoutNuit = setTimeout(function() {
                ...
                if (nuit.length === 0) {
                    $('#statusNuit').html('');
                    $('#nuit').removeClass('is-valid-custom is-invalid-custom');
                    return;
                }
                
                if (nuit.length !== 14) {
                    $('#statusNuit').html('<span class="status-badge status-error">NUIT deve ter 14 dígitos</span>');
                    $('#nuit').addClass('is-invalid-custom').removeClass('is-valid-custom');
                    return;
                }
                
                // Formatar NUIT para exibição
                const nuitFormatado = nuit.substr(0, 3) + '.' + nuit.substr(3, 3) + '.' + nuit.substr(6, 3) + '.' + nuit.substr(9, 5);
                $('#nuit').val(nuitFormatado);
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.valid) {
                            ...
                            $('#nuit').addClass('is-valid-custom').removeClass('is-invalid-custom');
                        } else {
        ...
        function validarCelularRealtime() {
            ...
            timeoutCelular = setTimeout(function() {
                ...
                if (celular.length === 0) {
                    $('#statusCelular').html('');
                    $('#celular').removeClass('is-valid-custom is-invalid-custom');
                    return;
                }
                ...
                if (celular.length === 9) {
                    ...
                    if (prefixos.includes(celular.substr(0, 2))) {
                        ...
                        $('#celular').addClass('is-valid-custom').removeClass('is-invalid-custom');
                    } else {
                        $('#statusCelular').html('<span class="status-badge status-error"><i class="fas fa-times-circle"></i> Prefixo inválido. Use 91-99</span>');
                        $('#celular').addClass('is-invalid-custom').removeClass('is-valid-custom');
                    }
        ...
        // Validar telefone fixo em tempo real
        function validarTelefoneRealtime() {
            ...
            timeoutTelefone = setTimeout(function() {
                ...
                if (telefone.length === 0) {
                    $('#statusTelefone').html('');
                    $('#telefone').removeClass('is-valid-custom is-invalid-custom');
                    return;
                }
                ...
                if (telefone.length === 9) {
                    ...
                    if (prefixos.includes(telefone.substr(0, 2))) {
                        ...
                        $('#telefone').addClass('is-valid-custom').removeClass('is-invalid-custom');
                    } else {
                        $('#statusTelefone').html('<span class="status-badge status-error"><i class="fas fa-times-circle"></i> Prefixo inválido. Use 22-26</span>');
                        $('#telefone').addClass('is-invalid-custom').removeClass('is-valid-custom');
                    }
        ...
        // Gerar sugestões de subdomínio baseado no nome
        function gerarSugestoesSubdominio() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        data.forEach(function(sug) {
                            html += '<span class="sugestao-item" onclick="$(\'#subdominio\').val(\'' + sug + '\'); verificarSubdominioRealtime();">' + sug + '</span>';
                        });
                        $('#sugestoesSubdominio').html(html);
                    }
                });
            } else {
                $('#sugestoesSubdominio').html('');
            }
        }
        ...
        function formatarCelular() {
            ...
            $('#celular').val(valor);
            validarCelularRealtime();
        }
        
        // Formatar telefone fixo enquanto digita
        function formatarTelefone() {
            let valor = $('#telefone').val().replace(/[^0-9]/g, '');
            if (valor.length > 9) valor = valor.substr(0, 9);
            if (valor.length >= 4 && valor.length <= 6) {
                valor = valor.substr(0, 3) + ' ' + valor.substr(3);
            } else if (valor.length >= 7) {
        ...
        $('#nome_escola').on('input', function() { 
            setTimeout(gerarSugestoesSubdominio, 300);
        });
        ...
        $('#celular').on('input', function() { formatarCelular(); });
        $('#telefone').on('input', function() { formatarTelefone(); });
