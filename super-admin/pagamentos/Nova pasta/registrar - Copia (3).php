...
function enviarWhatsApp($telefone, $mensagem) {
    // Integração com API de WhatsApp (ex: Twilio, WhatsApp Business API)
    // Por enquanto, retorna true simulando envio
    // $telefone = '244' . preg_replace('/[^0-9]/', '', $telefone);
    // file_get_contents("https://api.whatsapp.com/send?phone={$telefone}&text=" . urlencode($mensagem));
    return true;
}
    ...
    try {
        ...
        <head>
            ...
            <style>
                ...
                @media print {
                    ...
                    .comprovante-container { box-shadow: none; margin: 0; }
        ...
        <body>
            ...
            <div style="text-align: center; margin-top: 20px;" class="btn-print no-print">
                <button onclick="window.print()" style="padding: 10px 20px; background: #006B3E; color: white; border: none; border-radius: 5px; cursor: pointer; margin: 5px;">
        ...
        // ============================================
                ...
                /*    // No registrar.php, altere a linha do envio de e-mail
if ($enviar_email && !empty($dados['email']) && false) { // Adicione && false para desativar
...
*/
        ...
        if ($enviar_email && !empty($dados['email'])) {
            ...
            $mensagem_email = "
                ...
                <p>Em anexo segue o comprovante de pagamento em PDF.</p>
            ...
            $headers .= "From: SIGE Angola <noreply@sige.ao>\r\n";
            ...
            $mensagem_email .= "Content-Type: text/html; charset=utf-8\r\n\r\n";
            $mensagem_email .= $mensagem_email . "\r\n\r\n";
            $mensagem_email .= "--{$boundary}\r\n";
            $mensagem_email .= "Content-Type: text/html; name=\"{$numero_comprovante}.html\"\r\n";
            $mensagem_email .= "Content-Disposition: attachment; filename=\"{$numero_comprovante}.html\"\r\n\r\n";
            $mensagem_email .= file_get_contents($comprovante_file) . "\r\n\r\n";
            $mensagem_email .= "--{$boundary}--";
            
            mail($to, $assunto, $mensagem_email, $headers);
        }
        ...
        if ($enviar_whatsapp && !empty($dados['celular'])) {
            $mensagem_whatsapp = "🏫 *SIGE Angola - Comprovante de Pagamento*\n\n";
            $mensagem_whatsapp .= "*Escola:* " . $dados['nome'] . "\n";
            $mensagem_whatsapp .= "*Valor:* " . formatarMoeda($valor) . "\n";
            $mensagem_whatsapp .= "*Data:* " . formatarData($data_pagamento) . "\n";
            $mensagem_whatsapp .= "*Referente:* {$referente}\n";
            $mensagem_whatsapp .= "*Comprovante:* {$numero_comprovante}\n\n";
            $mensagem_whatsapp .= "Acesse o sistema para mais detalhes.\n";
            $mensagem_whatsapp .= "https://sige.ao\n\n";
            $mensagem_whatsapp .= "_Este é um e-mail automático, por favor não responda._";
                    ...
                    <div class="row mt-3">
                        <div class="col-md-12">
                            ...
                            <div class="form-check">
                                ...
                                <label class="form-check-label">
                                    <i class="fab fa-whatsapp"></i> Enviar comprovante por WhatsApp
                                </label>
                            </div>
                        </div>
                    </div>
