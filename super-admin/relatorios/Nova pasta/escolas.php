...
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <ul class="nav-menu">
            ...
            <li class="nav-item"><a href="escolas.php" class="nav-link active"><i class="fas fa-chart-line"></i> Relatórios</a></li>
            <li class="nav-item"><a href="financeiro.php" class="nav-link"><i class="fas fa-chart-pie"></i> Financeiro</a></li>
            <li class="nav-item"><a href="estatisticas.php" class="nav-link"><i class="fas fa-chart-bar"></i> Estatísticas</a></li>
    ...
    <div class="main-content">
        ...
        <div class="stats-grid">
            <div class="stat-card">
                <div class="stat-value"><?php echo count($escolas); ?></div>
                <div class="stat-label">Total de Escolas</div>
            </div>
            <div class="stat-card">
                <div class="stat-value"><?php echo $resumo_status['ativa'] ?? 0; ?></div>
                <div class="stat-label">Escolas Ativas</div>
            </div>
            <div class="stat-card">
                <div class="stat-value"><?php echo $resumo_status['trial'] ?? 0; ?></div>
                <div class="stat-label">Em Período Trial</div>
            </div>
            <div class="stat-card">
                <div class="stat-value"><?php echo $resumo_status['suspensa'] ?? 0; ?></div>
                <div class="stat-label">Escolas Suspensas</div>
            </div>
            <div class="stat-card">
                <div class="stat-value"><?php echo array_sum(array_column($escolas, 'total_usuarios')); ?></div>
                <div class="stat-label">Total de Usuários</div>
            </div>
            <div class="stat-card">
                <div class="stat-value">KZ <?php echo number_format(array_sum(array_column($escolas, 'total_recebido')), 2, ',', '.'); ?></div>
                    ...
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="table table-hover">
                                <thead>
                                    <tr><th>#</th><th>Escola</th><th>Subdomínio</th><th>Usuários</th></tr>
                                </thead>
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><?php echo $escola['subdominio']; ?>.sige.ao</td>
                                        <td><span class="badge bg-primary"><?php echo $escola['total_usuarios']; ?></span></td>
                                    </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="text-center mb-3">
                                    <h3><?php echo $resumo_status['ativa'] ?? 0; ?></h3>
                                    <p class="text-success">Ativas</p>
                                    <div class="progress">
                                        <div class="progress-bar bg-success" style="width: <?php echo ($resumo_status['ativa'] ?? 0) / max(count($escolas), 1) * 100; ?>%"></div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="text-center mb-3">
                                    <h3><?php echo $resumo_status['trial'] ?? 0; ?></h3>
                                    <p class="text-info">Em Trial</p>
                                    <div class="progress">
                                        <div class="progress-bar bg-info" style="width: <?php echo ($resumo_status['trial'] ?? 0) / max(count($escolas), 1) * 100; ?>%"></div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="text-center">
                                    <h3><?php echo $resumo_status['suspensa'] ?? 0; ?></h3>
                                    <p class="text-warning">Suspensas</p>
                                    <div class="progress">
                                        <div class="progress-bar bg-warning" style="width: <?php echo ($resumo_status['suspensa'] ?? 0) / max(count($escolas), 1) * 100; ?>%"></div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="text-center">
                                    <h3><?php echo $resumo_status['inativa'] ?? 0; ?></h3>
                                    <p class="text-danger">Inativas</p>
                                    <div class="progress">
                                        <div class="progress-bar bg-danger" style="width: <?php echo ($resumo_status['inativa'] ?? 0) / max(count($escolas), 1) * 100; ?>%"></div>
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div class="card">
            <div class="card-header">
                <i class="fas fa-list"></i> Lista de Escolas
                <span class="badge bg-secondary ms-2"><?php echo count($escolas); ?> registros</span>
            </div>
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><?php echo $i + 1; ?></td>
                                <td>
                                    <strong><?php echo htmlspecialchars($e['nome']); ?></strong><br>
    ...
    <script>
        ...
        new Chart(ctx1, {
            type: 'doughnut',
            data: {
                labels: ['Ativas', 'Trial', 'Suspensas', 'Inativas'],
                datasets: [{
                    data: [
                        <?php echo $resumo_status['ativa'] ?? 0; ?>,
                        <?php echo $resumo_status['trial'] ?? 0; ?>,
                        <?php echo $resumo_status['suspensa'] ?? 0; ?>,
                        <?php echo $resumo_status['inativa'] ?? 0; ?>
                    ],
        ...
        const ctx2 = document.getElementById('provinciaChart').getContext('2d');
        const provLabels = <?php echo json_encode(array_column($resumo_provincia, 'provincia')); ?>;
        const provValues = <?php echo json_encode(array_column($resumo_provincia, 'total')); ?>;
        ...
        // Gráfico de planos
        const ctx3 = document.getElementById('planoChart').getContext('2d');
        const planLabels = <?php echo json_encode(array_column($resumo_plano, 'plano_nome')); ?>;
        const planValues = <?php echo json_encode(array_column($resumo_plano, 'total')); ?>;
        ...
        const ctx4 = document.getElementById('crescimentoChart').getContext('2d');
        const anosLabels = <?php echo json_encode(array_column($crescimento_anual, 'ano')); ?>;
        const anosValues = <?php echo json_encode(array_column($crescimento_anual, 'total')); ?>;
        ...
        const ctx5 = document.getElementById('crescimentoMensalChart').getContext('2d');
        new Chart(ctx5, {
            type: 'bar',
            data: {
