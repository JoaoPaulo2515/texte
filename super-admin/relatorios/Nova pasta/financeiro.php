...
<head>
    ...
    <style>
        ...
        .badge-pago { background: #e8f5e9; color: #388e3c; }
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <ul class="nav-menu">
            ...
            <li class="nav-item"><a href="financeiro.php" class="nav-link active"><i class="fas fa-chart-pie"></i> Financeiro</a></li>
    ...
    <div class="main-content">
        ...
        <div class="stats-grid">
            <div class="stat-card">
                <div class="stat-value">KZ <?php echo number_format($resumo_geral['total_recebido'] ?? 0, 2, ',', '.'); ?></div>
                <div class="stat-label">Total Recebido</div>
                <small><?php echo $resumo_geral['qtd_pagos'] ?? 0; ?> pagamentos</small>
            </div>
            <div class="stat-card">
                ...
                <small><?php echo $resumo_geral['qtd_pendentes'] ?? 0; ?> pendentes</small>
            </div>
            <div class="stat-card">
                <div class="stat-value">KZ <?php echo number_format($inadimplencia['valor_total'] ?? 0, 2, ',', '.'); ?></div>
                <div class="stat-label">Inadimplência</div>
                <small>Média: <?php echo round($inadimplencia['dias_atraso_medio'] ?? 0); ?> dias</small>
            </div>
            <div class="stat-card">
                <div class="stat-value"><?php echo number_format(($resumo_geral['total_recebido'] > 0 ? ($resumo_geral['total_pendente'] / $resumo_geral['total_recebido'] * 100) : 0), 1); ?>%</div>
            ...
            <div class="col-md-4">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mt-3">
                            <?php foreach ($receita_planos as $plano): ?>
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                <span><?php echo $plano['plano_nome']; ?></span>
                                <span class="badge bg-secondary">KZ <?php echo number_format($plano['total'], 2, ',', '.'); ?></span>
                            </div>
                            <?php endforeach; ?>
                        </div>
                    </div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mt-3">
                            ...
                            foreach ($pagamentos_metodo as $metodo): ?>
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                <span><?php echo $metodos_nomes[$metodo['metodo_pagamento']] ?? ucfirst($metodo['metodo_pagamento']); ?></span>
                                <span><?php echo $metodo['quantidade']; ?> transações</span>
                                <span class="badge bg-info">KZ <?php echo number_format($metodo['total'], 2, ',', '.'); ?></span>
                            </div>
                            <?php endforeach; ?>
                        </div>
                    </div>
                    ...
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="table table-hover">
                                <thead>
                                    <tr><th>Mês</th><th>Valor Previsto</th><th>Status</th></tr>
                                </thead>
                                <tbody>
                                    <?php foreach ($previsao_receita as $prev): ?>
                                    <tr>
                                        <td><?php echo date('F/Y', strtotime($prev['mes'] . '-01')); ?></td>
                                        <td>KZ <?php echo number_format($prev['total_previsto'], 2, ',', '.'); ?></td>
                                        <td><span class="badge bg-warning">Pendente</span></td>
                                    </tr>
                                    <?php endforeach; ?>
                                    <?php if (empty($previsao_receita)): ?>
                                    <tr><td colspan="3" class="text-center">Nenhuma previsão para os próximos meses</td></tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
                        </div>
        ...
        <div class="card">
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover">
                        <thead>
                            <tr><th>#</th><th>Escola</th><th>Subdomínio</th><th>Nº Pagamentos</th><th>Total Pago</th></tr>
                        </thead>
                        <tbody>
                            <?php foreach ($top_escolas as $i => $escola): ?>
                            <tr>
                                <td><?php echo $i + 1; ?></td>
                                <td><strong><?php echo htmlspecialchars($escola['nome']); ?></strong></td>
                                <td><?php echo $escola['subdominio']; ?>.sige.ao</td>
                                <td><?php echo $escola['total_pagamentos']; ?></td>
                                <td>KZ <?php echo number_format($escola['total_pago'], 2, ',', '.'); ?></td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
    ...
    <script>
        ...
        new Chart(ctx1, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    data: <?php echo json_encode($dados_faturamento); ?>,
                    borderColor: '#006B3E',
        ...
        const planosLabels = <?php echo json_encode(array_column($receita_planos, 'plano_nome')); ?>;
        const planosValues = <?php echo json_encode(array_column($receita_planos, 'total')); ?>;
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        callbacks: {
                            label: function(context) {
                                return context.label + ': KZ ' + context.raw.toLocaleString('pt-AO', {minimumFractionDigits: 2});
                            }
                        }
                    }
                }
        ...
        const ctx3 = document.getElementById('metodosChart').getContext('2d');
        const metodosLabels = <?php 
            $labels = [];
            $values = [];
            foreach ($pagamentos_metodo as $m) {
                $metodos_nomes = ['dinheiro'=>'Dinheiro','transferencia'=>'Transferência','deposito'=>'Depósito','cartao'=>'Cartão','multicaixa'=>'Multicaixa'];
                $labels[] = $metodos_nomes[$m['metodo_pagamento']] ?? ucfirst($m['metodo_pagamento']);
                $values[] = $m['total'];
            }
