<?php
// super-admin/suporte.php - Gestão de Tickets de Suporte
        ...
        try {
            ...
            $stmt = $conn->prepare("
                INSERT INTO ticket_respostas (
                    ticket_id, usuario_id, mensagem, is_admin, created_at
                ) VALUES (
                    :ticket_id, :usuario_id, :mensagem, 1, NOW()
                )
            ");
            $stmt->execute([
                ':ticket_id' => $id,
...
$query = "
    SELECT t.*, e.nome as escola_nome, e.subdominio, e.logo,
...
if ($escola_id) {
    $query .= " AND t.escola_id = :escola_id";
...
// Buscar ticket para visualização
$ticket_view = null;
$respostas = [];
if ($action == 'view' && isset($_GET['id'])) {
    $stmt = $conn->prepare("
        SELECT t.*, e.nome as escola_nome, e.subdominio, e.email as escola_email, e.telefone as escola_telefone
        FROM tickets_suporte t
        JOIN escolas e ON e.id = t.escola_id
        WHERE t.id = :id
    ");
    ...
    if ($ticket_view) {
        $stmt = $conn->prepare("
            SELECT r.*, u.nome as usuario_nome, u.tipo
...
<head>
    ...
    <title>Suporte - Super Admin | SIGE SaaS</title>
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <ul class="nav-menu">
            ...
            <li class="nav-item"><a href="?page=suporte" class="nav-link active"><i class="fas fa-headset"></i> Suporte</a></li>
    ...
    <div class="main-content">
        ...
        <?php if ($action == 'view' && $ticket_view): ?>
            ...
            <div class="card">
                <div class="card-header">
                    <div>
                        <h3><i class="fas fa-ticket-alt"></i> Ticket #<?php echo $ticket_view['id']; ?> - <?php echo htmlspecialchars($ticket_view['assunto']); ?></h3>
                        <small>Escola: <?php echo htmlspecialchars($ticket_view['escola_nome']); ?> (<?php echo $ticket_view['subdominio']; ?>.sige.com)</small>
                    </div>
                    <a href="?page=suporte" class="btn btn-secondary btn-sm">
                ...
                <div class="card-body">
                    <div class="row mb-4">
                        <div class="col-md-3">
                            ...
                            ?>">
                                <?php echo ucfirst($ticket_view['prioridade']); ?>
                            </span>
                        </div>
                        <div class="col-md-3">
                            <strong>Status:</strong>
                            <span class="status-badge status-<?php echo $ticket_view['status']; ?>">
                                <?php echo ucfirst(str_replace('_', ' ', $ticket_view['status'])); ?>
                            </span>
                        </div>
                        <div class="col-md-3">
                            <strong>Categoria:</strong>
                            <?php echo ucfirst($ticket_view['categoria'] ?? 'Geral'); ?>
                        </div>
                        <div class="col-md-3">
                            <strong>Aberto em:</strong>
                            <?php echo date('d/m/Y H:i', strtotime($ticket_view['created_at'])); ?>
                        </div>
                    </div>
                    ...
                    <div class="message-bubble <?php echo $resposta['is_admin'] ? 'admin' : ''; ?>">
                        <strong>
                            <i class="fas <?php echo $resposta['is_admin'] ? 'fa-user-shield' : 'fa-user'; ?>"></i>
                            <?php echo $resposta['is_admin'] ? 'Suporte SIGE' : htmlspecialchars($resposta['usuario_nome']); ?>
                        </strong>
                    ...
                    <form method="POST">
                        ...
                        <a href="?page=suporte&action=fechar&id=<?php echo $ticket_view['id']; ?>" class="btn btn-secondary" onclick="return confirm('Fechar ticket?')">
                            <i class="fas fa-check"></i> Fechar Ticket
                        </a>
                    </form>
                    <?php endif; ?>
                </div>
        ...
        <?php else: ?>
            ...
            <div class="stats-grid">
                <div class="stat-card">
                    <div class="stat-value"><?php echo $stats['abertos']; ?></div>
                    <div>Tickets Abertos</div>
                </div>
                <div class="stat-card">
                    <div class="stat-value"><?php echo $stats['urgentes']; ?></div>
                    <div>Tickets Urgentes</div>
                </div>
                <div class="stat-card">
                    <div class="stat-value"><?php echo $stats['tempo_medio']; ?></div>
                    <div>Tempo Médio (horas)</div>
                </div>
            </div>
            
            <div class="card">
                <div class="card-header">
                    <h3><i class="fas fa-list"></i> Tickets de Suporte</h3>
                    <div class="btn-group">
                        <a href="?page=suporte" class="btn btn-sm btn-secondary">Todos</a>
                        <a href="?page=suporte&status=aberto" class="btn btn-sm btn-danger">Abertos</a>
                        <a href="?page=suporte&status=respondido" class="btn btn-sm btn-info">Respondidos</a>
                        <a href="?page=suporte&status=fechado" class="btn btn-sm btn-success">Fechados</a>
                    </div>
                </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover">
                            ...
                            <tbody>
                                ...
                                <tr class="ticket-item prioridade-<?php echo $ticket['prioridade']; ?>">
                                    <td>
                                        <strong>#<?php echo $ticket['id']; ?></strong>
                                    </td>
                                    <td>
                                        <strong><?php echo htmlspecialchars($ticket['escola_nome']); ?></strong><br>
                                        <small><?php echo $ticket['subdominio']; ?>.sige.com</small>
                                     </td>
                                    <td><?php echo htmlspecialchars($ticket['assunto']); ?></td>
                                    <td>
                                        <span class="badge bg-<?php 
                                            echo $ticket['prioridade'] == 'urgente' ? 'danger' : 
                                                ($ticket['prioridade'] == 'alta' ? 'warning' : 
                                                ($ticket['prioridade'] == 'media' ? 'info' : 'success')); 
                                        ?>">
                                    ...
                                    <td>
                                        <a href="?page=suporte&action=view&id=<?php echo $ticket['id']; ?>" class="btn btn-sm btn-primary">
